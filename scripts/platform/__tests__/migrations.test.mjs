...
const alphaManifest = readManifest(
  path.join(FIXTURES_ROOT, "alpha", "module.json")
)
const deltaManifest = readManifest(
  path.join(FIXTURES_ROOT, "delta", "module.json")
)

const DRIZZLE_CUSTOM_STUB =
...
function makeChild(journalEntryCount = 2) {
  ...
  const entries = Array.from({ length: journalEntryCount }, (_, idx) => ({
    idx,
    tag: `${idx}_kernel`,
  }))
  writeFileSync(
...
test("generateForModule roda check, generate --name <module>_baseline e db:check:journal nessa ordem", () => {
  ...
  const generated = generateForModule(child, alphaManifest, {
    catalogEntryRoot: path.join(FIXTURES_ROOT, "alpha"),
    run,
  })
...
test("generateForModule sem diff de schema devolve lista vazia — nenhum baseline fantasma para o lock", () => {
  ...
  assert.ok(
    !existsSync(
      path.join(child, "apps/api/drizzle/migrations", "0002_alpha_baseline.sql")
    )
  )
})
...
test("generateForModule aborta com exit 9 quando drizzle-kit check acusa drift, sem chamar generate", () => {
  ...
  const { run, calls } = makeStubRun([
    { status: 1, stdout: "drift detectado\n", stderr: "" },
  ])

  assert.throws(
    () =>
      generateForModule(child, alphaManifest, {
        catalogEntryRoot: path.join(FIXTURES_ROOT, "alpha"),
        run,
      }),
    (err) => {
...
test("generateForModule gera uma migração --custom por entrada de customMigrations e sobrescreve o arquivo registrado no journal", () => {
  ...
  const generated = generateForModule(child, deltaManifest, {
    catalogEntryRoot,
    run,
  })
    ...
    [
      ...
      [
        ...
        [
          ...
          "--custom",
          "--name",
          "delta_delta_events_append_only",
        ],
      ],
      ["pnpm", ["--filter", "api", "run", "db:check:journal"]],
    ]
  ...
  const shippedSql = readFileSync(
    path.join(
      catalogEntryRoot,
      "migrations/custom/01_delta_events_append_only.sql"
    ),
    "utf8"
  )
...
test("generateForModule com baseline sem diff grava o SQL custom no arquivo que o drizzle registrou — nada de órfão nem stub vazio", () => {
  ...
  assert.equal(
    readFileSync(
      path.join(migrationsDir, "0003_delta_delta_events_append_only.sql"),
      "utf8"
    ),
    shippedSql
  )
  assert.ok(
    !existsSync(
      path.join(migrationsDir, "0004_delta_delta_events_append_only.sql")
    )
  )
  ...
  assert.deepEqual(journal.entries.map((entry) => entry.tag).slice(3), [
...
test("generateForModule lança MigrationFailureError tipada (exit 9) quando o SQL custom da entrada não existe, antes de gerar a migração", () => {
  ...
  const manifest = {
    name: "delta",
    customMigrations: ["99_arquivo_inexistente.sql"],
  }
