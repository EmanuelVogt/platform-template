...
**Project-local, mandatory before you phrase anything**: read [`.specs/lessons-vocabulary.md`](../../../../.specs/lessons-vocabulary.md) and first search for an existing lesson that already says it:

```bash
python3 scripts/lessons.py list --status all --query "<keyword>"
```

If one matches in meaning, **reuse its `text` verbatim** — only `--feature` and `--source` change. That is what makes recurrence count. Phrasing it your own way silently creates a second candidate, and neither ever promotes. This project reached 21 candidates and zero confirmed lessons exactly that way (consolidated 2026-08-10).
...
Promotion is a **trigger, not an endpoint**. A lesson that stays as text is guidance that only applies while someone is running this skill — it does nothing for an ordinary edit outside the flow. In this repo the rules that actually held (contract enums, the closed `Select`, branch-only-in-worktree) all ended as something that blocks.
...
Deduplication is exact-after-normalization (lowercase, punctuation-stripped) — there are no embeddings (stdlib-only, zero-dependency by design). Near-duplicate lessons phrased differently will not merge and will each sit as separate candidates that never promote. Mitigation: follow the phrasing rules above and the closed vocabulary in [`.specs/lessons-vocabulary.md`](../../../../.specs/lessons-vocabulary.md). A future version may add embedding-based dedup.
