...
metadata:
  ...
  version: 3.2.0
  fork: 2026-08-17 — Execute delegated to cheap workers from 4 tasks up, wave/cluster parallelism over vertical slices (references/sub-agents.md); every artifact and payload in English regardless of the user's language (Critical Rule 6)
---
...
All of it in English, including `[feature]` (a short English kebab-case slug: `onboarding-email-retry`, not a slug in the user's language) — Critical Rule 6.
...
- **Roles:** orchestrator (planning window) · **workers** (one per cluster) · **scout** (finds code, returns `file:line`) · **runner** (runs the main window's gates — the Build gate and the Final gate — returning exit code + literal failures) · **Verifier** (fresh, independent). Workers run their own scoped gates, redirected to a log, and nest a scout for a question they cannot scope; the Verifier nests the runner for its Final gate; scouts and runners never nest further. Claude Code: `spec-worker`, `repo-scout`, `shell-runner`, `spec-verifier` under `.claude/agents/`; the `delegate-to-subagent` hook enforces the nesting.
...
- **Tier is judgement per dispatch, never hard-coded** — passed as `model` on every call: mechanics → low/haiku (payload forbids reformatting); everything else, root config/tooling/CI/docs/tests included → mid/sonnet (default); domain entities/transitions, transaction/outbox/ALS, migrations, contract regen, ADR-governed rule → high/opus. Verifier mid by default, high only for auth, payments, data integrity, or a rule the product's own domain doc marks critical (P0). Twice failed at one tier → one tier up. State the tier in the wave report. Table: sub-agents.md § *Model selection*.
