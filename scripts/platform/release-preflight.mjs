...
function previousStableTag({ repoRoot, exec }) {
  const result = exec("git", ["ls-remote", "--tags", "--refs", repoRoot, "v*"], { cwd: repoRoot });
...
function moduleVersionAt({ repoRoot, exec, ref, entryDir }) {
  ...
  const result = exec("git", ["show", `${ref}:${relDir}/module.json`], { cwd: repoRoot });
...
function entryChangedWithoutBump({ repoRoot, exec, previousTag, entryDir }) {
  const relDir = path.relative(repoRoot, entryDir);
  const diffResult = exec("git", ["diff", "--quiet", previousTag, "HEAD", "--", relDir], { cwd: repoRoot });
  if (diffResult.status === 0) return false;
  const previousVersion = moduleVersionAt({ repoRoot, exec, ref: previousTag, entryDir });
  if (previousVersion === undefined) return false;
  const currentVersion = moduleVersionAt({ repoRoot, exec, ref: "HEAD", entryDir });
...
export async function runPreflight({
  ...
  if (version !== latest) {
    log(`release-preflight — versão "${version}" não é a última seção do changelog ("${latest}")`);
    return EXIT_CODES.USAGE_ERROR;
  }

  const tagCheck = exec("git", ["tag", "-l", `v${version}`], { cwd: repoRoot });
  if (tagCheck.stdout.trim().length > 0) {
    log(`release-preflight — a tag "v${version}" já existe (nada novo para liberar)`);
...
if (isMain(import.meta.url, process.argv[1])) {
  ...
  if (messageIndex !== -1) {
    process.stdout.write(`${preflightMessage({ version: argv[messageIndex + 1] })}\n`);
