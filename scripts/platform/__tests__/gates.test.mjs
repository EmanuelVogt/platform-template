...
test("GAT-07: nenhum package.json de shell web tem script test*", () => {
  assert.ok(WEB_PACKAGE_JSON_PATHS.length > 0, "esperava ao menos um shell web em apps/");
  for (const manifestPath of WEB_PACKAGE_JSON_PATHS) {
    ...
    assert.deepEqual(testScripts, [], `${path.relative(ROOT_DIR, manifestPath)} ainda expõe script test*`);
  }
});
