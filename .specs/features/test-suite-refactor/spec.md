...
- [ ] Gates exist and run: CI (check → unit → int → e2e → contract → coverage-all), pre-push unchanged (Docker-free) with the 95 % unit/web bar on the kernel-only denominator.
...
| Raising the nyc combined floor (85/51/90/90) | unchanged — still `test:cov:all`, off-hook |
...
| Rewriting `docs/back/back-arch.md` § Testes | owned by v1 HBK-02; this feature updates `docs/test/testing.md` and links |
...
| GA-5 lint | `eslint-plugin-jest` (api), `@vitest/eslint-plugin` + `testing-library` + `jest-dom` (web), plus a local `no-existence-only-assert` rule | the handbook rule L-007 is prose today and is violated in the tree | n |
| GA-6 CI | one `.github/workflows/ci.yml`; the existing `catalog.yml` is extended, never duplicated; pre-push stays Docker-free | `testing.md` § CI describes a pipeline that never ran | n |
...
**Independent Test**: `pnpm --filter api test:e2e` green; `harness-hygiene.spec.ts` green and failing on a seeded violation.
...
1. WHEN `apps/api/src/shared/test/unit/` is read THEN it SHALL export `mockOf<T>(partial?): jest.Mocked<T>`, `fixedClock(iso?)`, `fakeRequestContext(partial?)` with the kernel defaults and `fakeLogger()`, AND a use-case spec's dependency factory SHALL be typed — `Record<string, any>` SHALL not appear in any spec and `as never` / `as unknown as` SHALL appear only under `shared/test/**`.
...
**Independent Test**: `pnpm --filter api test` and `test:int` green; the guard spec's typed-deps and `fromProps` checks green.
...
1. WHEN `pnpm lint` resolves the config for a test file THEN `no-focused-tests`, `no-disabled-tests`, `expect-expect` and `no-conditional-expect` SHALL be active as errors (api via `eslint-plugin-jest`, web via `@vitest/eslint-plugin`), and web test files SHALL additionally resolve `testing-library` and `jest-dom` recommended rules as errors.
...
4. WHEN the e2e suites run with `--randomize` THEN they SHALL pass — no `it` depends on a previous `it`, and the "seed master" pseudo-test is removed.
...
**Independent Test**: RuleTester suite of the local rule green; `pnpm --filter api test:e2e -- --randomize` green; the `it`-count probe reports no regression.
...
**Independent Test**: `pnpm --filter web test` green; the web half of the guard spec green.
...
1. WHEN a push or pull request runs CI THEN jobs `check`, `unit`, `int`, `e2e`, `contract` and `coverage-all` SHALL run, `contract` SHALL fail on a dirty `openapi.json`, `e2e` SHALL run with `--randomize`, and all jobs SHALL use the Node of `.nvmrc` and the pnpm of `packageManager`.
...
3. WHEN pre-push runs THEN it SHALL stay Docker-free (typecheck, migrations check, api `test:cov`, web `test:cov`) and SHALL block below the coverage bar.
4. WHEN `turbo.json` is read THEN `test:cov`, `test:cov:all` and `test:watch` SHALL be declared with correct `outputs` and with `cache: false` for Docker-bound tasks.

**Independent Test**: workflow green on the feature branch; a forced sub-threshold coverage fixture blocks `git push`.
...
1. WHEN pre-push runs api `test:cov` and web `test:cov` THEN any metric below 95 % (statements, branches, functions, lines) SHALL block the push, and at or above 95 % coverage SHALL never block (COV-01..03).
...
5. WHEN the fills land THEN they SHALL target `apps/api/src/**` and `apps/web/src/**` on the post-v1 denominator, and the ratchet SHALL raise the jest and vitest thresholds to 95 and switch the pre-push api task to `test:cov` last (COV-11).
...
| Idempotency / retry | `waitFor` and `drainOutbox` are the only retry primitives; suites are order-independent under `--randomize` |
...
| LNT-01 | P1 proof — jest/vitest/testing-library/jest-dom rules active as errors (AC1) | test | Tasks | In Tasks |
...
| STR-02 | P1 proof — order independence under `--randomize` (AC4) | gate | Tasks | In Tasks |
...
- [ ] `pnpm --filter api test:e2e -- --randomize` green; every tier green; the `it`-count probe reports no regression beyond the single allowed removal.
