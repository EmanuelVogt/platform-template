-- Trilha de auditoria por trigger genérica (mecanismo, extraído da migration
-- 0003_audit_trail.sql do platform — ver ADR 0040). Captura no banco, não
-- app-layer: parte dos writes não passa pela entidade (update em massa,
-- replace DELETE+INSERT, backfill) — captura app-layer ficaria cega ou
-- fabricaria histórico. A aplicação só LÊ a trilha
-- (api/infrastructure/tables/audit-entry.table.ts é view-only; esta tabela
-- não é gerada pelo drizzle-kit, é criada aqui).
--
-- Este arquivo NÃO anexa o trigger a nenhuma tabela de módulo — cada módulo
-- que cria uma tabela auditável chama `SELECT audit.attach(...)` na própria
-- migration (ver `## Dados` do README desta entrada).
...
BEGIN
  ...
  -- sempre carimba updatedAt — sem a exclusão qualquer save full-row vira ruído).
  IF TG_OP = 'UPDATE' THEN
...
--    Cada módulo com tabela auditável chama isto na própria migration, ex.:
--    SELECT audit.attach('meu_schema', 'minha_tabela', '{id}', '{}');
...
-- 6) Busca da trilha por `q` (autor/entidade/alterações). O trgm no conteúdo
