...
import {
  checkAdvisoryRange,
  checkAdvisoryRequired,
} from "../advisory-required.mjs"
...
test("aceita quando há um ADV staged com module correspondente (exit esperado: 0)", () => {
  const result = checkAdvisoryRequired({
    ...
    stagedAdvisories: [
      {
        path: "docs/advisories/ADV-20260901-01.md",
...
test("rejeita quando o ADV staged é de outro module (exit esperado: 1)", () => {
  const result = checkAdvisoryRequired({
    stagedFiles: [
      "catalog/identity/single-tenant/api/session.service.ts",
      "docs/advisories/ADV-20260901-02.md",
    ],
    commitMessage: "feat(identity): ajusta expiração de sessão",
    stagedAdvisories: [
      {
        path: "docs/advisories/ADV-20260901-02.md",
        module: "attachment/single-tenant",
      },
    ],
  })
  assert.equal(result.ok, false)
...
test("range: o trailer do último commit não isenta um commit anterior do mesmo PR", () => {
  ...
  assert.deepEqual(result.failures, [
    {
      sha: "1111111111111111111111111111111111111111",
      missing: ["identity/single-tenant"],
    },
  ])
})
    ...
    commits: [
      {
        ...
        files: [
          "catalog/tag/web/core/labels.ts",
          "docs/advisories/ADV-20260901-03.md",
        ],
        message: "feat(tag): novo campo",
        advisories: [
          { path: "docs/advisories/ADV-20260901-03.md", module: "tag" },
        ],
      },
      {
...
test("range: reprova cada commit que falha, não só o primeiro", () => {
  const result = checkAdvisoryRange({
    commits: [
      {
        sha: "aaaaaaaaaa",
        files: ["catalog/tag/api/tag.service.ts"],
        message: "feat(tag): a",
        advisories: [],
      },
      {
        sha: "bbbbbbbbbb",
        files: ["catalog/audit/api/trail.ts"],
        message: "feat(audit): b",
        advisories: [],
      },
    ],
  })
...
test("resolve o nome real da entrada quando o próprio tier api aninha um segmento api (catalog/<entry>/api/api/**)", () => {
  const result = checkAdvisoryRequired({
    stagedFiles: [
      "catalog/notification/api/api/contracts/notification.contract.ts",
    ],
...
test("resolve variante + tier api aninhado sem colidir com o segmento de variante (catalog/identity/single-tenant/api/api/**)", () => {
  const result = checkAdvisoryRequired({
    stagedFiles: [
      "catalog/identity/single-tenant/api/api/contracts/session.contract.ts",
    ],
