...
const BASE_AREAS = new Set(["kernel-api", "kernel-web", "harness", "docs", "ci-infra"]);
...
const MODULE_PATH_RE = /^apps\/(?:api\/src\/modules|web\/src\/entities)\/([^/]+)\//;
...
const SECRET_PATTERNS = [
  ...
  { re: /\b[a-z][a-z0-9+.-]*:\/\/[^/\s:@]+:[^@\s]+@/i, label: "URL com credencial embutida" },
  { re: /\b(?:senha|password|secret|token|api[_-]?key)\b\s*[:=]\s*["'][^"']{6,}/i, label: "atribuição de segredo" },
];
...
export function classifyPath(rawPath, lockModules) {
  ...
  if (clean.startsWith("apps/api/drizzle/migrations/")) {
    ...
    return { ok: false, reason: "migração do produto (só NNNN_kernel_* é da plataforma)" };
  }
  if (PLATFORM_PREFIXES.some((prefix) => clean.startsWith(prefix)) || PLATFORM_FILES.has(clean)) {
    return { ok: true };
  }
  const moduleMatch = MODULE_PATH_RE.exec(clean);
  if (moduleMatch) {
    if (lockModules.has(moduleMatch[1])) return { ok: true, entry: moduleMatch[1] };
    return { ok: false, reason: "módulo de negócio do produto (fora do .platform-modules.lock)" };
  }
...
export function parseDraft(md, { lockModules }) {
  ...
  if (!match) return { problems: ["frontmatter YAML ausente (title/type/area/paths)"] };
  ...
  else if (title.length > MAX_TITLE) problems.push(`title acima de ${MAX_TITLE} caracteres`);

  if (!TYPES.has(frontmatter.type)) {
    problems.push(`type inválido: ${frontmatter.type ?? "(nenhum)"} — use bug|improvement`);
  }

  const area = typeof frontmatter.area === "string" ? frontmatter.area : "";
  if (!BASE_AREAS.has(area) && !CATALOG_AREA_RE.test(area)) {
    problems.push(`area inválida: ${area || "(nenhuma)"} — use kernel-api|kernel-web|harness|docs|ci-infra|catalog/<entry>`);
  } else if (area.startsWith("catalog/") && !lockModules.has(area.slice("catalog/".length))) {
    problems.push(`area ${area}: entrada não instalada neste produto (.platform-modules.lock)`);
  }

  const paths = Array.isArray(frontmatter.paths)
    ? frontmatter.paths.filter((p) => typeof p === "string" && p.trim() !== "")
    : [];
  if (paths.length === 0) problems.push("paths vazio — aponte os arquivos da plataforma afetados");
  for (const p of paths) {
    const verdict = classifyPath(p, lockModules);
    if (!verdict.ok) problems.push(`caminho fora do escopo da plataforma: ${p} — ${verdict.reason}`);
  }

  const body = match[2].trim();
  if (!body) problems.push("corpo vazio — seções What / Evidence / Suggested fix");

  return { draft: { title, type: frontmatter.type, area, paths, body }, problems };
}
...
export function composeIssue({ draft, origin, modules }) {
  ...
  if (modules.length > 0) {
    footer.push(`- modules: ${modules.map((m) => `${m.name}@${m.version}`).join(", ")}`);
  }
  return `${draft.body}\n\n${footer.join("\n")}\n`;
}
...
export function githubRepoOf(source) {
  ...
  const match = /^(?:https:\/\/|git@)github\.com[/:]([^/\s]+)\/([^/\s]+?)(?:\.git)?\/?$/.exec(expanded);
...
export async function feedbackCommand({ draftPath, options = {}, cwd = process.cwd() }) {
  if (!draftPath) {
  ...
  const modules = Object.entries(lock.modules ?? {}).map(([name, entry]) => ({ name, version: entry.version }));
  ...
  if (problems.length > 0 || secrets.length > 0) {
    for (const problem of problems) process.stderr.write(`bloqueado: ${problem}\n`);
    for (const hit of secrets) {
      process.stderr.write(`bloqueado: possível segredo na linha ${hit.line} (${hit.label}) — redija e rode de novo\n`);
    }
  ...
  if (!origin) {
    lines.push("origem do template desconhecida (.copier-answers.yml ausente ou sem _src_path) — abra a issue no upstream com o corpo acima");
  } else if (!repo) {
    lines.push(`origem sem repositório GitHub reconhecível (${origin.source}) — abra a issue no upstream correspondente com o corpo acima`);
  } else {
    lines.push("com o OK do usuário, abra a issue no upstream:", `  ${ghCommand}`);
    lines.push(
      ...(url
