...
const ADVISORIES_README_PATH = path.join(
  TESTS_DIR,
...
test("parseAdvisory lança erro quando kind não está no enum permitido", () => {
  assert.throws(
    ...
    (error) =>
      error instanceof AdvisoryParseError && /kind inválido/.test(error.message)
  )
})
...
test("parseAdvisory lança erro quando severity não está no enum permitido", () => {
  assert.throws(
    ...
    (error) =>
      error instanceof AdvisoryParseError &&
      /severity inválido/.test(error.message)
  )
})
...
test("parseAdvisory lança erro quando não há frontmatter", () => {
  assert.throws(
    ...
    (error) =>
      error instanceof AdvisoryParseError && /frontmatter/.test(error.message)
  )
})
...
test("computePending: advisory aponta variant não instalada do módulo não fica pendente", () => {
  const advisory = parseAdvisory(
    advisoryMd({ module: "identity/multi-tenant" })
  )
  const result = computePending(lockWith(installedIdentity), [advisory], [])
  assert.deepEqual(result.pending, [])
})
...
test("computePending: advisory de kernel fica pendente quando a versão instalada bate com affects", () => {
  const advisory = parseAdvisory(
    advisoryMd({ module: "kernel", affects: ">=2.0.0 <2.1.0" })
  )
  const result = computePending({ modules: {} }, [advisory], [], {
...
test("loadAdvisories: ADV-*.md com schema inválido falha alto (não é engolido)", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "advisories-test-"))
  writeFileSync(
    path.join(dir, "ADV-20260901-02.md"),
    advisoryMd({ kind: "cosmetic" })
  )
  assert.throws(
    () => loadAdvisories(dir),
    (error) =>
...
test("readLedger: extrai os ids das linhas do ledger", () => {
  ...
  writeFileSync(
    filePath,
    "- ADV-20260901-01 — 2026-09-02 — abc123\n- ADV-20260815-02 — 2026-08-16 — def456\n"
  )
