...
test("defaultCatalogRef lê _src_path e _commit de .copier-answers.yml", () => {
  ...
  assert.equal(
    defaultCatalogRef(answersPath),
    "gh:EmanuelVogt/platform-template#v1.0.0"
  )
})

test("defaultCatalogRef retorna undefined quando o arquivo não existe", () => {
  assert.equal(
    defaultCatalogRef(path.join(tmpdir(), "nao-existe-copier-answers.yml")),
    undefined
  )
})
...
test("defaultCatalogRef desce em catalog/ quando _src_path é um caminho local (não git)", () => {
  ...
  assert.equal(
    defaultCatalogRef(answersPath),
    `${path.join(repoRoot, "catalog")}#v0.2.0-87-g48e6855`
  )
})
...
test("resolveCatalog resolve a fonte default de um _src_path local (raiz do template) até catalog/", () => {
  ...
  const result = resolveCatalog(undefined, {
    copierAnswersPath: path.join(answersDir, ".copier-answers.yml"),
  })
...
test("resolveCatalog reaproveita o cache de um ref imutável (tag) sem clonar de novo", () => {
  ...
  assert.ok(
    existsSync(sentinel),
    "cache de tag deveria ser reusado, não reclonado"
  )
  assert.ok(existsSync(path.join(second.root, "alpha", "module.json")))
})
...
test("resolveCatalog revalida um ref móvel (branch): a segunda chamada enxerga o que andou no origin", () => {
  ...
  pushNewOriginFile(
    workDir,
    bareDir,
    path.join("catalog", "alpha", "novo.txt"),
    "novo conteúdo"
  )
...
test("splitCatalogRef separa um caminho local#<ref> em (source, gitRef)", () => {
  const dir = "/tmp/algum-diretorio"
  assert.deepEqual(splitCatalogRef(`${dir}#v1.2.3`), {
    source: dir,
    gitRef: "v1.2.3",
  })
  assert.equal(isGitRef(dir), false)
...
test("splitCatalogRef corta na PRIMEIRA # quando a própria fonte contém #", () => {
  assert.deepEqual(splitCatalogRef("/tmp/weird#dir#v1"), {
    source: "/tmp/weird",
    gitRef: "dir#v1",
  })
})
...
test("expandGitShorthand expande gh:/gl: para a URL https clonável", () => {
  assert.equal(
    expandGitShorthand("gh:owner/repo"),
    "https://github.com/owner/repo.git"
  )
  assert.equal(
    expandGitShorthand("gh:owner/repo.git"),
    "https://github.com/owner/repo.git"
  )
  assert.equal(
    expandGitShorthand("gl:owner/repo"),
...
test("expandGitShorthand não altera URLs completas nem caminhos locais", () => {
  assert.equal(
    expandGitShorthand("https://github.com/o/r.git"),
    "https://github.com/o/r.git"
  )
  assert.equal(
    expandGitShorthand("git@github.com:o/r.git"),
    "git@github.com:o/r.git"
  )
  assert.equal(
    expandGitShorthand("file:///tmp/repo.git"),
    "file:///tmp/repo.git"
  )
  assert.equal(
    expandGitShorthand("/tmp/algum-diretorio"),
    "/tmp/algum-diretorio"
  )
})
