...
test("growth: .md.jinja handbooks and AGENTS.md.jinja count as handbooks", () => {
  assert.equal(runHook(edit(doc("docs/dev/deploy.md.jinja"), lines(40))).status, 2);
  assert.equal(runHook(edit(doc("AGENTS.md.jinja"), lines(40))).status, 2);
});

test("new file: handbook cap 80, ADR cap 60", () => {
  assert.equal(runHook(write(doc("docs/dev/new-thing.md"), lines(80))).status, 0);
  assert.equal(runHook(write(doc("docs/dev/new-thing.md"), lines(81))).status, 2);
...
test("rationale: heading or pt-BR phrase in a handbook is blocked; code spans are ignored", () => {
  assert.equal(runHook(edit(doc("docs/agents/workflow.md"), "## Why this design\ntext")).status, 2);
  assert.equal(runHook(edit(doc("docs/agents/workflow.md"), "## Alternatives\ntext")).status, 2);
  assert.equal(runHook(edit(doc("docs/agents/workflow.md"), "Optamos por isso.")).status, 2);
  assert.equal(runHook(edit(doc("docs/agents/workflow.md"), "run `porque --why`")).status, 0);
});

test("scope: ADRs accept rationale; advisories, .specs and source are not handbooks", () => {
  assert.equal(runHook(edit(doc("docs/adr/0001-x.md"), "## Why\nporque sim")).status, 0);
  assert.equal(runHook(write(doc("docs/advisories/ADV-20260823-01.md"), `${lines(120)}\nporque`)).status, 0);
  assert.equal(runHook(write(doc(".specs/features/x/spec.md"), `${lines(200)}\nporque`)).status, 0);
...
test("bash: shell writes into a handbook are blocked, reads and git mv pass", () => {
  ...
  assert.equal(runHook(bash('sed -i "" "s/a/b/" docs/agents/workflow.md')).status, 2);
...
test("escape hatches: PLATFORM_DOCS_LEAN_OFF=1 and unparsable stdin never block", () => {
  assert.equal(runHook(edit(doc("docs/code-quality.md"), lines(99)), { PLATFORM_DOCS_LEAN_OFF: "1" }).status, 0);
