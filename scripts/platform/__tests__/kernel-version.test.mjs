...
test("readLatestChangelogVersion rejects a changelog with no version heading", () => {
  ...
  try {
    assert.throws(
      ...
      (err) =>
        err instanceof ChangelogVersionMissingError &&
...
function withChangelogFixture(build) {
  return withTmpDir((root) =>
    writeFileSync(path.join(root, "changelog.md"), build ?? CHANGELOG_FIXTURE)
  )
}
...
test("readChangelogSection slices from its heading to the next '## vX.Y.Z' heading", () => {
  ...
  try {
    const section = readChangelogSection(
...
test("readChangelogSection rejects a version with no matching heading", () => {
  ...
  try {
    assert.throws(
      ...
      (err) =>
        err instanceof ChangelogSectionMissingError && err.version === "9.9.9"
    )
  } finally {
    cleanup(dir)
  }
