...
const DOCS = ["README.md.jinja", ".github/README.md", "docs/dev/local-environment.md"];
...
test("every documented pnpm command resolves in a manifest", () => {
  ...
  for (const doc of DOCS) {
    for (const { scope, token, raw } of pnpmCommands(read(doc))) {
      ...
      assert.ok(resolved, `${doc}: \`${raw}\` names \`${token}\`, which is not a pnpm builtin or a script in the matching manifest`);
    }
  }
});
...
test("db:bootstrap and db:seed:demo are named in no doc", () => {
  for (const doc of DOCS) {
    ...
    assert.doesNotMatch(content, /db:bootstrap/, `${doc} still names db:bootstrap`);
    assert.doesNotMatch(content, /db:seed:demo/, `${doc} still names db:seed:demo`);
  }
});

test("db:seed is named in no doc and is not an apps/api script", () => {
  for (const doc of DOCS) {
    assert.doesNotMatch(read(doc), /db:seed\b(?!:demo)/, `${doc} still names db:seed`);
  }
  assert.equal(Object.hasOwn(readJson("apps/api/package.json").scripts, "db:seed"), false);
});

test("no apps/api script targets the absent src/seeds directory", () => {
  const apiScripts = readJson("apps/api/package.json").scripts;
  for (const [name, command] of Object.entries(apiScripts)) {
    assert.doesNotMatch(command, /src\/seeds/, `script "${name}" still targets the absent apps/api/src/seeds`);
  }
});
