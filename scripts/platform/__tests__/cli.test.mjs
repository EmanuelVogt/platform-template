...
async function installAlpha(child) {
  ...
  return run(["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT], {
    cwd: child,
    run: stubRun,
  })
}
...
test("module add instala alpha com sucesso: copia arquivo, grava lock e registries", async () => {
  ...
  const exitCode = await run(
    ["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT],
    {
      cwd: child,
  ...
  assert.equal(
    readFileSync(alphaDestFile(child), "utf8"),
    "export class AlphaModule {}\n"
  )

  const apiTestCall = calls.find((call) => call.args.includes("--project"))
  assert.deepEqual(apiTestCall.args, [
    "vitest",
    "run",
    "--project",
    "api",
  ...
  const platformModules = readFileSync(
  ...
  assert.match(
    platformModules,
    /export const PLATFORM_MODULES = \[resolvePlatformModule\(AlphaModule\)\] as const;/
  )
})
...
test("module add grava o env do módulo em apps/api/.env (não na raiz), mesmo sem .env pré-existente", async () => {
  ...
  const exitCode = await run(
    ["module", "add", "epsilon", "--catalog-ref", CATALOG_ROOT],
  ...
  assert.match(
    readFileSync(path.join(child, "apps/api/.env"), "utf8"),
    /EPSILON_SECRET=troque-me/
  )
  assert.match(
    readFileSync(path.join(child, "apps/api/.env.example"), "utf8"),
    /EPSILON_SECRET=troque-me/
  )
})
...
test("module add retorna exit 3 quando o catálogo é inacessível", async () => {
  ...
  const exitCode = await run(
    ["module", "add", "alpha", "--catalog-ref", missingCatalog],
    { cwd: child, run: stubRun }
  )
...
test("module add retorna exit 3 quando o módulo não existe no catálogo", async () => {
  ...
  const exitCode = await run(
    ["module", "add", "nao-existe", "--catalog-ref", CATALOG_ROOT],
    { cwd: child, run: stubRun }
  )
...
test("module add retorna exit 4 quando o módulo já está instalado", async () => {
  ...
  const { result: exitCode, output } = await captureOutput("stderr", () =>
    run(["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT], {
      cwd: child,
      run: stubRun,
    })
  )
...
test("module add retorna exit 5 quando faltam dependências sem --with-deps", async () => {
  ...
  const { result: exitCode, output } = await captureOutput("stderr", () =>
    run(["module", "add", "beta", "--catalog-ref", CATALOG_ROOT], {
      cwd: child,
      run: stubRun,
    })
  )
...
test("module add --with-deps instala alpha antes de beta", async () => {
  ...
  const exitCode = await run(
    ["module", "add", "beta", "--catalog-ref", CATALOG_ROOT, "--with-deps"],
    {
      cwd: child,
      run: stubRun,
    }
  ...
  const alphaIdx = calls.findIndex((call) =>
    call.args.includes("alpha_baseline")
  )
...
test("module add retorna exit 6 quando o destino já existe, e --force sobrescreve", async () => {
  ...
  const forcedExitCode = await run(
    ["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT, "--force"],
    {
      cwd: child,
      run: forceRun,
    }
...
test("module add retorna exit 7 quando os testes falham após a cópia, mantendo os arquivos", async () => {
  const child = makeChild()
  const { run: stubRun } = makeStubRun({
    "modules/alpha": { status: 1, stdout: "", stderr: "falhou\n" },
  })
...
test("module add --skip-tests não executa pnpm test", async () => {
  const child = makeChild()
  const { run: stubRun, calls } = makeStubRun({
    "modules/alpha": { status: 1, stdout: "", stderr: "" },
  })

  const exitCode = await run(
    ["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT, "--skip-tests"],
    {
      cwd: child,
      run: stubRun,
    }
...
test("module add retorna exit 9 quando drizzle-kit check falha, mantendo os arquivos", async () => {
  const child = makeChild()
  const { run: stubRun } = makeStubRun({
    "drizzle-kit check": { status: 1, stdout: "drift\n", stderr: "" },
  })
...
test("module add --dry-run não escreve nada em disco", async () => {
  ...
  const { result: exitCode, output } = await captureOutput("stdout", () =>
    run(
      ["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT, "--dry-run"],
      { cwd: child, run: stubRun }
    )
  )
...
test("module add --rollback remove arquivos, lock e não afeta outros módulos", async () => {
  ...
  const { run: stubRun } = makeStubRun()
  const exitCode = await run(
    ["module", "add", "alpha", "--catalog-ref", CATALOG_ROOT, "--rollback"],
    {
      cwd: child,
      run: stubRun,
    }
...
test("module adopt grava o lock sem copiar arquivos", async () => {
  const child = makeChild()
  const sentinel =
    "// já presente manualmente (v0.2)\nexport class AlphaModule {}\n"
  mkdirSync(path.dirname(alphaDestFile(child)), { recursive: true })
  writeFileSync(alphaDestFile(child), sentinel, "utf8")

  const exitCode = await run(
    ["module", "adopt", "alpha", "--catalog-ref", CATALOG_ROOT],
    { cwd: child }
  )
  ...
  assert.ok(
    lockOf(child).modules.alpha.files[0].path.endsWith("alpha.module.ts")
  )
})
...
test("advisory detect executa o comando detect e retorna exit 1 quando afetado", async () => {
  ...
  const { run: stubRun, calls } = makeStubRun({
    "meu-detector": { status: 1, stdout: "", stderr: "" },
  })

  const exitCode = await run(["advisory", "detect", "ADV-20260901-01"], {
    advisoriesDir,
    run: stubRun,
  })
...
test("advisory detect retorna exit 0 quando o comando detect não acusa afetação", async () => {
  ...
  const { run: stubRun } = makeStubRun({
    "meu-detector": { status: 0, stdout: "", stderr: "" },
  })

  const exitCode = await run(["advisory", "detect", "ADV-20260901-02"], {
    advisoriesDir,
    run: stubRun,
  })
...
test("status --json não reporta o advisory de kernel quando a versão instalada está fora do affects", async () => {
  ...
  const { output } = await captureOutput("stdout", () =>
    run(["status", "--json"], { cwd, fetchTags: () => ["v2.1.0"] })
  )
...
test("status (texto) mostra a falha do feed sem derrubar o resto da saída (spec edge case)", async () => {
  ...
  writeFileSync(
    path.join(cwd, ".platform-modules.lock"),
    JSON.stringify({ modules: { alpha: { version: "1.0.0" } } })
  )
  ...
  const { output } = await captureOutput("stdout", () =>
    run(["status", "--json"], {
      ...
      fetchFeed: () => ({
        ...
        skipped: [
          {
            file: "ADV-20260901-09.md",
            reason: "campo obrigatório ausente: fix",
          },
        ],
      }),
      now: Date.parse("2026-08-23T00:00:00Z"),
...
test("status (texto) nomeia o arquivo do feed ignorado por falha de parse (spec edge case)", async () => {
  ...
  assert.match(
    output,
    /advisories: 1 arquivo\(s\) do feed remoto ignorado\(s\) — ADV-20260901-09\.md \(campo obrigatório ausente: fix\)/
  )
})
