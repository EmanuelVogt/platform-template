...
export function withEnvDefaults(run, defaults, { only } = {}) {
  return (command, args = [], options = {}) => {
    ...
    const applied = Object.fromEntries(Object.entries(defaults).map(([key, value]) => [key, process.env[key] ?? value]));
    return run(command, args, { ...options, env: { ...process.env, ...applied, ...options.env } });
  };
}
