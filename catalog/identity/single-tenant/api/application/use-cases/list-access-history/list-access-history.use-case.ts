...
export class ListAccessHistoryUseCase
  implements UseCaseContract<ListAccessHistoryInput, ListAccessHistoryOutput>
{
  constructor(
  ...
  async execute(input: ListAccessHistoryInput): Promise<ListAccessHistoryOutput> {
    const { userId } = requireAuth(this.ctx)
    const result = await this.authEvents.listByUser(userId, input, ACCESS_HISTORY_EVENT_TYPES)
