...
const betaManifest = readManifest(
  path.join(FIXTURES_ROOT, "beta", "module.json")
)

const REPO_CATALOG = path.join(
  path.dirname(fileURLToPath(import.meta.url)),
  "../../../catalog"
)

test("resolveDeps --with-deps resolve dependência que mora sob uma variante", () => {
  const attachment = readManifest(
    path.join(REPO_CATALOG, "attachment", "module.json")
  )
...
test("resolveDeps lança CyclicDependencyError e nomeia o nó em um self-edge", () => {
  const manifest = readManifest(
    path.join(FIXTURES_ROOT, "self-cycle", "module.json")
  )
  const lock = { modules: {} }
  try {
...
test("resolveDeps lança CyclicDependencyError e nomeia os dois nós em um ciclo de dois nós", () => {
  const manifest = readManifest(
    path.join(FIXTURES_ROOT, "cycle-a", "module.json")
  )
  const lock = { modules: {} }
  try {
    resolveDeps({ catalogRoot: FIXTURES_ROOT, manifest, lock, withDeps: true })
...
test("resolveDeps lança CyclicDependencyError e nomeia a cadeia completa em um ciclo de três nós", () => {
  const manifest = readManifest(
    path.join(FIXTURES_ROOT, "cycle-x", "module.json")
  )
  const lock = { modules: {} }
  try {
    resolveDeps({ catalogRoot: FIXTURES_ROOT, manifest, lock, withDeps: true })
...
test("resolveDeps detecta um ciclo alcançável apenas por um nó que não é o alvo da instalação", () => {
  const manifest = readManifest(
    path.join(FIXTURES_ROOT, "root-cycle", "module.json")
  )
  const lock = { modules: {} }
  try {
    resolveDeps({ catalogRoot: FIXTURES_ROOT, manifest, lock, withDeps: true })
...
test("resolveDeps resolve um diamante sem falso positivo de ciclo, com ordem topológica válida", () => {
  const manifest = readManifest(
    path.join(FIXTURES_ROOT, "diamond-a", "module.json")
  )
  const lock = { modules: {} }
  const result = resolveDeps({
    catalogRoot: FIXTURES_ROOT,
  ...
  assert.deepEqual([...result.order].sort(), [
    "diamond-a",
    "diamond-b",
    "diamond-c",
    "diamond-d",
...
test("planCopy monta a lista from/to seguindo a convenção api/** -> apps/api/src/modules/<name>/**", () => {
  ...
  const { files, conflicts } = planCopy(
    path.join(FIXTURES_ROOT, "alpha"),
    alphaManifest,
    { targetRoot: dir }
  )
...
test("planCopy reporta conflito quando o destino já existe", () => {
  ...
  assert.deepEqual(conflicts, [
    path.join(dir, "apps/api/src/modules/alpha/alpha.module.ts"),
  ])
})
