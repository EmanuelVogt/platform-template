...
import { FeedUnreachableError, fetchRemoteAdvisories, mergeAdvisories } from "../lib/advisory-feed.mjs";
...
function fakeExec({ advisories = { "ADV-20260821-01.md": ADVISORY_MD }, tagDate = "2026-08-21T10:00:00-03:00" } = {}) {
  const calls = [];
  const exec = (cmd, args) => {
    calls.push(args);
    if (args[0] === "clone") {
...
test("fetchRemoteAdvisories: fresh fetch clones, parses and populates the cache", () => {
  ...
  const result = fetchRemoteAdvisories(SOURCE, TAG, { cacheRoot, exec, now: 1_000 });
  ...
  const cached = JSON.parse(readFileSync(path.join(cacheRoot, cacheFile), "utf8"));
...
test("fetchRemoteAdvisories: a corrupt cache plus a failing exec throws FeedUnreachableError", () => {
  const cacheRoot = mkdtempSync(path.join(tmpdir(), "feed-cache-"));
  fetchRemoteAdvisories(SOURCE, TAG, { cacheRoot, exec: fakeExec().exec, now: 1_000 });
...
test("mergeAdvisories: the remote copy of a duplicated id wins", () => {
  ...
  const remote = [{ id: "ADV-1", severity: "high" }, { id: "ADV-3", severity: "high" }];
