...
### T49f: The child's `pnpm test:db` — 21 failures unmasked by T49c/T49e — **DONE 2026-08-25** (`5ad2050`, `faf3508`, `200f665`)
...
- [x] The A commit stages **no `catalog/` path**, so no advisory is due on it (`CODE_PATH_RE` matches only `catalog/<entry>/{api,web,migrations,parity}/`)
- [x] **B/C in one commit for the identity entry**, with `ADV-20260825-03` staged in it — one entry, one advisory. `audit`'s int-spec, if it needs one, is a separate commit with `ADV-20260825-04`
- [x] `auth-csrf-none.e2e-spec.ts`'s `afterAll` is guarded regardless of the B ruling — an unguarded teardown masks the real cause of any future setup failure
- [x] **The seam fix alone takes A from 12 to 8, not to 0** — surveyed at `7aa1c6c`. Neither int-spec sets `APP_TIMEZONE`, and both bake `-03:00` into the **expected** bucket instant via a local `saoPaulo()` helper (identity `:16-18`, audit `:11-13`), so the assertions are Brasília-shaped and still miss under the UTC default. Survive the seam fix on absolute-instant grounds: identity `:154`, audit `:113`/`:152`/`:159`. Still fail: identity `:88`/`:117`/`:132`/`:170`, audit `:59`/`:89`/`:123`/`:137`. The fix those eight want is for the spec to **declare** the timezone it is testing, which also makes the names true again — **but `resolved ??=` (`bucket-sql.ts:48,57`) memoises once per PROCESS**, so a `beforeAll` may land after the first resolve and may leak across files sharing a worker. **How the timezone is injected is the real design work in A; the seam itself is one line**
- [x] **B is an ABSENT `API_ORIGIN`, not a cross-host one** — `auth-csrf-none.e2e-spec.ts:33-39` spreads `...process.env` and overrides only `COOKIE_SAMESITE`/`COOKIE_SECURE`/`COOKIE_NAME`/`CSRF_SECRET`; it never sets `API_ORIGIN`, and `apps/api/src/shared/test/env.ts` sets none either. Ruling B is satisfied by **adding** `API_ORIGIN: "http://localhost:5173"` (the `WEB_ORIGIN` that `env.ts:10` sets) to that literal
- [x] **CORRECTION, do not re-derive: `auth-session.e2e-spec.ts:48` IS mechanical.** The earlier note calling it a behaviour question was wrong. The test *sends* `Cookie: rit_session=…` (`:44`) while the app's cookie is `app_session`, so `auth.middleware.ts` sees no recognised cookie, takes the early `if (!raw)` return and emits no `Set-Cookie` — exactly what the `SPEC_DEVIATION` comment at `:23-31` documents. Renaming `:44` makes the middleware see an *invalid* cookie and emit the clearing header, fixing `:48` too. **No behaviour call is owed**
- [x] **C is 17 sites across 6 files, not 7, and there is a THIRD cookie name (`rit_csrf`)** — full list in `child-testdb-failures.txt` § 4. The e2e fixture is **already** renamed (`shared/test/env.ts:17-18` sets `app_session`/`app_device`); only the specs are stale. **Both comments claiming "e2e-env usa COOKIE_NAME=rit_session"** (`auth-login:47`, `auth-session:47`) assert something false and must be deleted or corrected, not left beside a renamed literal. **`rit_csrf` needs a check first**: T53 made the CSRF cookie configurable (`app_csrf` default) rather than a literal, so `auth-csrf-none:60`/`:67` may need the config value, not a hardcoded string
- [x] An advisory per entry touched, staged in the same commit (see T49c's Done-when for the mechanics); `ADV-20260825-03` is free
- [x] Test count: **21 failures -> 0, measured**, with no test deleted merely to make it pass. The child's `pnpm test:db` went from `Test Files 8 failed | 63 passed (71); Tests 19 failed | 488 passed` to `Test Files 1 failed | 70 passed (71); Tests 1 failed | 511 passed (512)`. Every one of the 21 is gone; the single remaining failure is **not one of them and not this entry's** — see the line below
- [x] Gate passes: `pnpm catalog:check` exit 0, i.e. the eight-command Build gate at **8/8 — measured at `676c36e`**. `check`, `test`, `test:scripts` (687/687), `catalog:test`, `catalog:typecheck`, `catalog:lint`, `format:check`, `catalog:check` all exit 0; the child renders and runs `Test Files 245 passed (245) / Tests 1751 passed (1751)` unit and `Test Files 71 passed (71) / Tests 512 passed (512)` db. **The first run of the gate read 7/8 and it was contention, not a defect** — a second `catalog:check` from the sibling session shared the Docker host, and `catalog/tag/api/__e2e__/tags.e2e-spec.ts` > *"purge de tag fora da lixeira → 409"* died with `Error: Parse Error: Expected HTTP/, RTSP/ or ICE/`, an HTTP client reading a non-HTTP response rather than a failed assertion. Isolated, it passes. **Do not run `catalog:check` concurrently with another session's** — the failure it invents looks like a real e2e bug

**Result — the three commits, 2026-08-25**

| Commit | Scope | Advisory |
| --- | --- | --- |
| `5ad2050` | kernel: `appTimeZone` reads `process.env.APP_TIMEZONE`; memoisation moves from the **value** to the **warning** | none due — no `catalog/` path staged |
| `faf3508` | identity: B (`API_ORIGIN: E2E_ORIGIN`), the guarded `afterAll` in **both** describes, C's 17 sites, the int-spec's declared timezone | `ADV-20260825-03`, staged in it |
| `200f665` | audit: the int-spec declares its timezone | `ADV-20260825-04`, staged in it |
| `676c36e` | the two advisories pass `brand-hygiene` and `docs-shipped-paths` | n/a (docs + gate only) |

**The design call inside ruling A, made here because the survey left it open.** The ruling fixed *where* the timezone is read; `resolved ??=` memoised **once per process**, so a `beforeAll` could land after the first resolve and leak across files sharing a worker. Resolved by memoising the **warning** instead of the value: `appTimeZone()` re-reads `process.env.APP_TIMEZONE` on every call and a `warned` flag keeps the fallback log once per process. `bucket-sql.spec.ts:17-27` asserts exactly what it asserted before and passes unchanged; a sibling test now proves the re-read. This is what lets both int-specs declare `APP_TIMEZONE` in a `beforeAll` without depending on file order — and it removes the trap rather than working around it.
