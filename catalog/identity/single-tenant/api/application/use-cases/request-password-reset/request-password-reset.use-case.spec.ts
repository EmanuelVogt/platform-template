...
function makeDeps(over: Record<string, any> = {}) {
  ...
  return { uc, users, verificationTokens, tokens, outbox, authEvents, clock, ctx }
}
...
describe("RequestPasswordResetUseCase", () => {
  ...
  it("cooldown expirado: lastResetRequestedAt antigo o suficiente → fluxo completo", async () => {
    ...
    await t.uc.execute({ email: "carol@example.com" })
    expect(t.verificationTokens.invalidateAllForUser).toHaveBeenCalledWith("u-1", "password_reset")
