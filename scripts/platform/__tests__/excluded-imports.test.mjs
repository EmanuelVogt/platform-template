...
import { findExcludedImports, parseExcludeList } from "../lib/copier-exclude.mjs";

const TESTS_DIR = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(TESTS_DIR, "..", "..", "..");

test("parseExcludeList lê a lista _exclude real de copier.yml, nunca uma cópia hard-coded", () => {
  const excludeList = parseExcludeList(readFileSync(path.join(REPO_ROOT, "copier.yml"), "utf8"));
...
test("findExcludedImports falha numa injeção deliberada de import de lib/lint.mjs a partir de cli.mjs", () => {
  ...
  writeFileSync(path.join(root, "scripts", "platform", "cli.mjs"), 'import { discoverEntries } from "./lib/lint.mjs";\n');
  writeFileSync(path.join(root, "scripts", "platform", "lib", "lint.mjs"), "export function discoverEntries() {}\n");
  const excludeList = ["scripts/platform/lib/lint.mjs"];

  const offenders = findExcludedImports({ repoRoot: root, excludeList });

  assert.deepEqual(offenders, [
    { file: "scripts/platform/cli.mjs", specifier: "./lib/lint.mjs", resolved: "scripts/platform/lib/lint.mjs" },
  ]);
});
