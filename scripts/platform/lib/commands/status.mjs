...
export function collectStatus({
  ...
  if (!origin) {
    template = {
      source: undefined,
      error: ".copier-answers.yml ausente ou sem _src_path",
    }
  } else {
    let tags = []
    let error
    ...
    template = {
      ...
      ...computeTemplateStatus({ commit: origin.commit, tags }),
      error,
    }
  }
  ...
  if (origin && !offline && template.behind?.length > 0 && template.latest) {
    try {
      ...
      if (feed?.tagDate) {
        template.latestPublishedDaysAgo = ageDays(
          feed.tagDate.slice(0, 10),
          now
        )
      }
  ...
  try {
    ...
    advisories = {
      noLock,
      pending: pending.map((a) => toStatusAdvisory(a, now)),
    }
  } catch (err) {
...
function formatAdvisoryEntry(a) {
  if (typeof a.ageDays !== "number")
    return `${a.id} (${a.severity} ${a.module})`
...
function formatAdvisories(advisories) {
  ...
  if (advisories.noLock)
    return "advisories: sem .platform-modules.lock — rode pnpm platform module adopt"
...
function formatFeedSkipped(advisories) {
  ...
  const list = advisories.feedSkipped
...
export async function statusCommand({
  ...
  if (status.template.error && !status.template.source) {
    ...
    if (typeof status.template.latestPublishedDaysAgo === "number") {
      lines.push(
        `latest ${status.template.latest} published ${status.template.latestPublishedDaysAgo} days ago`
      )
    }
    if (status.template.feedError)
      lines.push(`template: feed não consultado — ${status.template.feedError}`)
    if (status.template.error)
      lines.push(`template: remoto não consultado — ${status.template.error}`)
    if (options.offline)
      lines.push("template: --offline, última versão não consultada")
  }
