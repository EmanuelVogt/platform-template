...
test("stableTagsFromLsRemote keeps only vX.Y.Z tags, sorted numerically", () => {
  assert.deepEqual(stableTagsFromLsRemote(LS_REMOTE), [
    "v1.2.0",
    "v2.0.0",
    "v2.1.0",
    "v10.0.0",
...
test("parseInstalledVersion reads a tag and a describe past the tag", () => {
  assert.deepEqual(parseInstalledVersion("v2.0.0"), {
    version: "v2.0.0",
    aheadBy: 0,
  })
  assert.deepEqual(parseInstalledVersion("v2.0.0-3-gabc1234"), {
    version: "v2.0.0",
    aheadBy: 3,
  })
...
test("computeTemplateStatus lists the tags after the installed one", () => {
  ...
  assert.deepEqual(
    computeTemplateStatus({ commit: "v10.0.0", tags }).behind,
    []
  )
  assert.equal(
    computeTemplateStatus({ commit: "deadbeef", tags }).installed,
    undefined
  )
  assert.equal(
    computeTemplateStatus({ commit: "v2.0.0", tags: [] }).latest,
    undefined
  )
})
...
test("formatTemplateStatus states up to date, behind, or unknown", () => {
  ...
  assert.equal(
    formatTemplateStatus(
      source,
      computeTemplateStatus({ commit: "v2.0.0", tags: ["v2.0.0"] })
    ),
  ...
  assert.match(
    formatTemplateStatus(
      source,
      computeTemplateStatus({ commit: "v2.0.0", tags: [] })
    ),
    /latest=\(desconhecida\)$/
  )
})
...
test("buildTemplateBehindReport appends one line per pending kernel advisory, first fix line only (FEED-02)", () => {
  ...
  assert.equal(
    lines[0],
    "template behind: installed v2.0.0 (installed from a commit 2 past that tag), latest v2.1.0 — 1 tag(s): v2.1.0"
  )
  assert.equal(
    lines[2],
    "ADV-20260823-01 bug high kernel — fix: copier update to >= v2.1.0"
  )
})
...
test("readTemplateOrigin reads _src_path and _commit from the copier answers", () => {
  ...
  assert.deepEqual(readTemplateOrigin(answersPath), {
...
test("listRemoteStableTags expands the gh: shorthand and wraps failures", () => {
  ...
  assert.deepEqual(calls[0], [
    ...
    [
      ...
      "--tags",
      "--refs",
      "https://github.com/acme/platform-template.git",
      "v*",
    ],
  ])
  assert.throws(
...
test("cachedRemoteStableTags hits the network once per TTL and per source", () => {
  ...
  const first = cachedRemoteStableTags("gh:acme/t", {
    cachePath,
    now: 1_000,
    fetchTags,
  })
  const second = cachedRemoteStableTags("gh:acme/t", {
    cachePath,
    now: 2_000,
    ttlMs: 10_000,
    fetchTags,
  ...
  cachedRemoteStableTags("gh:acme/t", {
    cachePath,
    now: 20_000,
    ttlMs: 10_000,
    fetchTags,
  ...
  cachedRemoteStableTags("gh:other/t", {
    cachePath,
    now: 20_001,
    ttlMs: 10_000,
    fetchTags,
...
test("collectStatus joins template, lock modules and pending advisories", () => {
  ...
  const status = collectStatus({
    cwd,
    fetchTags: () => ["v1.2.0", "v2.0.0", "v2.1.0"],
  })
...
test("collectStatus survives an unreachable remote and a missing lock", () => {
  ...
  const offline = collectStatus({
    cwd,
    offline: true,
    fetchTags: () => assert.fail("must not fetch"),
  })
...
function answersFixture(body) {
  ...
  if (body !== undefined)
    writeFileSync(path.join(dir, ".copier-answers.yml"), body)
  return dir
}

test("readTemplateVersion resolves a plain tag to its bare semver", () => {
  const cwd = answersFixture(
    "_commit: v2.2.1\n_src_path: gh:acme/platform-template\n"
  )
...
test("readTemplateVersion returns undefined for a dirty ref it cannot parse, instead of a garbage semver", () => {
  const cwd = answersFixture(
    "_commit: v2.2.1-4-gabc1234-dirty\n_src_path: gh:acme/platform-template\n"
  )
