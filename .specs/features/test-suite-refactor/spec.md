...
2. WHEN a test body's only assertions are `toBeDefined`, `toBeTruthy`, `toBeUndefined`, `toBeFalsy`, `resolves/rejects.toBeDefined`, `not.toThrow()` **or `not.toThrow(<matcher>)`** THEN the local rule `no-existence-only-assert` SHALL report it; WHEN the same body also asserts a concrete value or declares `expect.assertions(n)` THEN it SHALL NOT report. **Corrected 2026-08-25 (`784d1af`) — the original exemption had it backwards.** Measured in Vitest: `expect(fn).not.toThrow(X)` **passes when a different error is thrown**, so it is strictly weaker than the argument-less form, which fails on any throw. Exempting it steered authors to the weaker assertion, and the Verifier proved the loss by mutation — a `TypeError` seeded on the valid path of `assertValidPermissionSet` left four `not.toThrow(InvalidPermissionSetError)` assertions green, the four whose entire job was to prove a valid set is accepted. The rule judges the **body**, not the assertion: a `not.toThrow(<matcher>)` guard standing beside a concrete-value assertion is still legitimate.
...
- WHEN `no-existence-only-assert` meets `expect(fn).not.toThrow(SomeError)` as a body's **sole** assertion THEN it SHALL report — **corrected 2026-08-25**: the with-matcher form is weaker than the argument-less one, not stronger. It SHALL NOT report when the body also asserts a concrete value or declares `expect.assertions(n)`.
