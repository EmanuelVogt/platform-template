...
function buildFixtureRepo({ changelog = CHANGELOG, entries = {} } = {}) {
  ...
  for (const [entryName, moduleJson] of Object.entries(entries)) {
    ...
    writeFileSync(path.join(entryDir, "module.json"), JSON.stringify(moduleJson));
  }
  return root;
}
...
function fakeExec({ tagList = "", lsRemote = "", diffStatus = () => 0, showAt = () => undefined } = {}) {
  return (command, args) => {
    ...
    if (sub === "show") {
      ...
      return content === undefined ? { status: 128, stdout: "" } : { status: 0, stdout: content };
    }
...
test("runPreflight fails with TEST_FAILURE when an entry changed without a module.json version bump", async () => {
  const dir = buildFixtureRepo({ entries: { widget: { name: "widget", version: "1.0.0" } } });
  try {
    const logs = [];
    const exitCode = await runPreflight({
      version: "2.1.0",
...
test("runPreflight passes an entry that changed together with a module.json version bump", async () => {
  const dir = buildFixtureRepo({ entries: { widget: { name: "widget", version: "1.1.0" } } });
  try {
    const exitCode = await runPreflight({
      version: "2.1.0",
      repoRoot: dir,
      exec: fakeExec({
        lsRemote: "abc\trefs/tags/v2.0.0\n",
        diffStatus: () => 1,
        showAt: (ref) => JSON.stringify({ name: "widget", version: ref === "v2.0.0" ? "1.0.0" : "1.1.0" }),
      }),
      log: () => {},
    });
