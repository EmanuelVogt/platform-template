...
25. (T29) The api statements floor is **86.1**, not the 86.2 that the T18 reading implied. Istanbul's `percent()` — the function Vitest's threshold checker calls, verified live during T29 — **truncates** to two decimals instead of rounding, so the same fractions (884/1008 statements, 849/960 lines) print 87.69/88.43 in one run and 87.70/88.44 in another reading of the same data. There is no coverage drift between T18 and T29; the calibration follows the tool's literal output, since that is the number the gate itself compares against. Floors written: 86.1 / 72.7 / 89.8 / 86.9; web untouched.
26. (Verifier round 1) The CAT-02 and DOC-02 probes were declared too broadly in `spec.md` and matched legitimate prose: `rg -c 'jest\.' apps/api catalog` hit the five `catalog/*/CHANGELOG.md` migration notes (`apps/api` itself clean), and the DOC-02 `rg` hit the five `ADV-20260821-*` advisories, the codemod's own source and fixtures (`scripts/platform/jest-to-vitest*`) and `gates.test.mjs:53`'s `assert.equal(manifest.jest, undefined)`. All are files whose subject *is* Jest — the same class as `docs/dev/template-changelog.md`, which the probe already excluded. Probes tightened with `--glob '!*.md'` (CAT-02) and `--glob '!docs/advisories/*' --glob '!scripts/platform/jest-to-vitest*'` plus the absence-assertions named in the allowed set (DOC-02); no AC outcome weakened, the probes still discriminate over `apps`, `packages`, `docs` outside the advisories, the agent cards and the catalog READMEs.
27. (F6) `catalog/audit/api/domain/audit-coverage.int-spec.ts` now re-applies tag's attach migration in `beforeAll` (and detaches in `afterAll`, trigger latency being the same reason the identity detach exists). This is a spec deviation on purpose: the spec's AC3 assumes every AUDITED table carries `audit_row` after a combined install, and the shipped design says the opposite — no automatic retro-attach, the owner module attaches. The test therefore simulates the manual step the advisory now documents, instead of asserting a behaviour the catalog deliberately does not have. The assertion itself was not weakened.
...
- [x] AD-027 and AD-028 `active` (AD-028 wording: `api-int` include `{src,test}/**/*.int-spec.ts`); AD-012 marked superseded by AD-027
- [x] `design.md` § Spike results carries the measured numbers and floors from T29
- [x] `test-suite-refactor`: T5/T31/T37/T39, GA-5/GA-6, AD-023 wording, Test Coverage Matrix and Gate Check Commands re-baselined to the Vitest commands; the "blocked behind vitest-migration" note removed from its Handoff entry
- [x] feature folder moved to `.specs/features/done/vitest-migration/`, Handoff entries to `handoff-archive.md`; `feat/vitest-migration` merged into `main` (`--no-ff`); the semver tag is the owner's call
...
| 5 | C9 T28 (sonnet, exclusive, `full-unit`) | DONE · worker gate PASS (`pnpm install --frozen-lockfile && pnpm check && pnpm test && pnpm test:scripts` exit 0 — 399 vitest tests, 225 script tests) · **Build gate PASS** (`catalog:lint` 0 · `catalog:typecheck` 0 · `test:scripts` 225/225 · `turbo typecheck lint --filter=api` 0). `jest`, `@swc/jest`, `@types/jest`, `nyc` gone from `apps/api`; `@vitest/coverage-v8` gone from `apps/web` (root owns coverage, AD-027); lockfile regenerated. No deviation | `f431564` |
| 6 | C10 T29 (sonnet) | DONE · worker gate PASS (`pnpm test:coverage` with Docker, 89 files/519 tests, exit 0) · **Build gate PASS** (`catalog:lint` 0 · `catalog:typecheck` 0 · `test:scripts` 225/225 · `turbo typecheck lint --filter=api --filter=web` 0). Measured `apps/api/src/**` **87.69 / 74.21 / 91.3 / 88.43** → floors **86.1 / 72.7 / 89.8 / 86.9**; web untouched (64/56/61/64). Sensitivity proven once (statements floor 92.7 → exit 1, reverted). Deviation 25: the floor on statements is 86.1, not the 86.2 implied by T18 — istanbul's `percent()`, which Vitest's threshold checker calls, **truncates** to two decimals, so T18 read 87.70/88.44 for the same fractions (884/1008, 849/960); calibration follows the tool's literal output and there is no coverage drift. `design.md` § Spike results carries the numbers | `bb64e82` |
| 7 (fixes) | F6 (opus) ∥ F7 (haiku) + orchestrator probe fix | **Verifier round 1 = FAIL** (21/24 ACs clean, sensor 3/3 killed, no test-count drop; Final gate exit 7 at `catalog:check`). Three gaps closed. **F6** (blocker, CAT-03): `tag.tags` had no `audit_row` trigger in the **combined** 5-entry child — `resolveInstallOrder` with no requested entry yields notification, identity, tag, audit, attachment (LIFO readdir stack, `catalog-graph.mjs:58` + `lint.mjs:95`), so `0007_tag_audit_attach_tags.sql` runs before `0009_audit_audit_trail_capture.sql`, tag's `pg_proc` guard finds no `audit.attach` and skips. Retro-attach from audit was rejected on evidence — both entries' READMEs and both SQL files state there is no automatic re-attach — so the identity precedent was followed: new `catalog/audit/api/testing/reattach-tag-tables.ts` (guarded by `to_regclass('tag.tags')`, a no-op for standalone `catalog:check audit`), reattach/detach around `audit-coverage.int-spec.ts`, assertion untouched (Deviation 27); `ADV-20260821-05` gained the manual `SELECT audit.attach('tag','tags',…)` step for products that installed tag before audit — the real product hole this exposed. Combined `pnpm catalog:check` exit 0 (check 5/5 · test 201 files/1237 · test:db 68 files/463, was 1 failed/462). **F7** (major): `ADV-20260821-03` gained the `ALTER COLUMN created_at SET DEFAULT clock_timestamp()` instruction for identity 1.x, mirrored into the numbered steps (Deviation 24b closed). **Probes** (minor): Deviation 26. **Build gate PASS** (`catalog:lint` 0 · `catalog:typecheck` 0 · `test:scripts` 225/225 · `turbo typecheck lint --filter=api` 0). Open follow-up, out of this feature's scope: `resolveInstallOrder`'s root order is a readdir/LIFO artifact with no rationale — a sibling that opts into an optional provider (tag → `audit.attach`) can install before it | F7 `4f6e376` · probes `00351a1` · F6 `348606f` |
| Verifier | round 1 sonnet → FAIL · round 2 same agent resumed → **PASS** | 24/24 ACs match the spec outcome, 0 spec-precision gaps. Final gate exit 0 in a single run — 9/9 steps, with `catalog:check` (68 files/463) and `template:smoke` (4/4) green for the first time (both were unreached in round 1). Sensor: round 1's 3/3 mutations killed stands. The Verifier re-derived F6's root cause independently and confirmed the "no automatic retro-attach" design predates this feature (`catalog/tag/README.md:53`, `ec14e1f`), and re-ran both re-declared probes itself. Lesson L-013 (`spec_deviation`) recorded. One nit at PASS → F8: the `ALTER COLUMN` fragment in `ADV-20260821-03` was not copy-pasteable without its `ALTER TABLE` prefix | `validation.md` · F8 `87dc7dd` (both fragments now `ALTER TABLE identity.user_professional_services ALTER COLUMN created_at SET DEFAULT clock_timestamp();`, schema-qualified per the module's `pgSchema("identity")`) |
| Closeout | T30 (orchestrator, no worker) | DONE · merged into `main` `--no-ff` at `bc63070` (5 doc/spec conflicts resolved by re-expressing the branch's content in English over main's post-`653d757` files); post-merge sanity gate PASS (`pnpm install --frozen-lockfile` 0 · `test:scripts` 225 · `pnpm test` 76 files/399 tests · `typecheck` 0). AD-027 + AD-028 `active`, AD-012 superseded; `test-suite-refactor` re-baselined; folder archived under `.specs/features/done/`. **No tag, no push — the owner's call.** | `bc63070` + this commit |
