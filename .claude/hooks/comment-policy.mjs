...
for (const t of flat) {
  if (/eslint-disable/.test(t)) {
    violations.push({ line: t, why: 'suppressing lint is forbidden — fix the code' });
  } else if (/^(\/\/|\/\*|\*)\s*(TODO|FIXME)\b/i.test(t) && !/\(.*(20\d\d|#\d+|issue).*\)/i.test(t)) {
    violations.push({ line: t, why: 'TODO/FIXME without a measurable condition (date/issue) is forbidden' });
