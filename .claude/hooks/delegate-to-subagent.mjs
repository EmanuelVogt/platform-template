...
const stateFile = (sessionId) =>
  path.join(tmpdir(), `platform-delegate-${sessionId}.json`)
...
const sessionId = data.agent_id
  ? `${baseSessionId}-${data.agent_id}`
  : baseSessionId
...
const pathArgs = (rest) =>
  rest
    ...
    .filter(
      (token) =>
        token.includes("/") ||
        (token.startsWith(".") && existsSync(path.resolve(cwd, token)))
    )
...
const harnessScoped = (rest) => {
  ...
  return args.every((token) => {
    ...
    return (
      rel !== "" &&
      HARNESS_DIRS.some((dir) => rel === dir || rel.startsWith(`${dir}/`))
    )
  })
}
...
const classify = (command) => {
  for (const statement of command.split(/&&|\|\||;|\n/)) {
    ...
    if (cmd === "npx" && RUN_CMDS.has(path.basename(rest[0] ?? "")))
      return "run"
    if (cmd === "pnpm") {
...
const readBytesOf = (abs, offset, limit) => {
  ...
  if (size <= READ_SCAN_CAP)
    return Buffer.byteLength(
      lines.slice(start - 1, start - 1 + limit).join("\n")
    )
  return Math.min(
    size,
    Math.round((head.length / Math.max(1, lines.length - 1)) * limit)
  )
}
...
const summarize = (s) =>
  (s.length > 140 ? `${s.slice(0, 140)}…` : s).replace(/\s+/g, " ")
...
const commitReadBytes = () => {
  ...
  writeState(sessionId, {
    ...state,
    readBytes: (state.readBytes ?? 0) + pendingReadBytes,
  })
}
...
if (tool === "Grep" || tool === "Glob") {
  ...
  const budget = data.agent_id
    ? READ_BYTES_FREE_PER_AGENT
    : READ_BYTES_FREE_PER_TURN
