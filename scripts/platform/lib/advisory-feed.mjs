...
import { AdvisoryParseError, isAdvisoryFilename, parseAdvisory } from "./advisories.mjs";
...
function cacheFilePath(cacheRoot, source, tag) {
  return path.join(cacheRoot, `platform-template-feed-${hashRef(`${source}#${tag}`)}.json`);
}
...
function readValidCache(cacheFile, source, tag, ttlMs, now) {
  ...
  try {
    ...
    if (cached.source === source && cached.tag === tag && now - cached.fetchedAt < ttlMs) {
      return cached;
    }
  } catch {
...
export function fetchRemoteAdvisories(
  source,
  tag,
  { cacheRoot = tmpdir(), ttlMs = 24 * 60 * 60 * 1000, timeoutMs = 8000, exec = execFileSync, now = Date.now() } = {},
) {
  const cacheFile = cacheFilePath(cacheRoot, source, tag);
  const cached = readValidCache(cacheFile, source, tag, ttlMs, now);
  if (cached) {
    return { tag, tagDate: cached.tagDate, advisories: cached.advisories, skipped: cached.skipped, fromCache: true };
  }
  ...
  try {
    ...
    exec("git", ["sparse-checkout", "set", "docs/advisories"], { cwd: dest, stdio: "pipe", timeout: timeoutMs });
    const tagDate = String(
      exec("git", ["log", "-1", "--format=%cI"], { cwd: dest, stdio: "pipe", encoding: "utf8", timeout: timeoutMs }),
    ).trim();

    const { advisories, skipped } = parseAdvisoriesDir(path.join(dest, "docs", "advisories"));

    mkdirSync(cacheRoot, { recursive: true });
    writeFileSync(cacheFile, JSON.stringify({ source, tag, tagDate, advisories, skipped, fetchedAt: now }));
