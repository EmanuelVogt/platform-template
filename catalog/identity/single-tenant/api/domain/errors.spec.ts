...
describe('errors de domínio identity', () => {
  ...
  it('InvalidCredentialsError: status 401, type estável, título pt-BR', () => {
    ...
    expect(err.type).toBe('https://errors.example.com/auth/invalid-credentials');
  ...
  it('WeakPasswordError: status 422', () => {
    ...
    expect(err.type).toBe('https://errors.example.com/auth/weak-password');
  ...
  it('InvalidResetTokenError: status 400', () => {
    ...
    expect(err.type).toBe('https://errors.example.com/auth/invalid-reset-token');
  });

  it('SessionNotFoundError: status 404', () => {
    const err = new SessionNotFoundError();
    expect(err.status).toBe(404);
    expect(err.type).toBe('https://errors.example.com/auth/session-not-found');
  });

  it('RateLimitedError: status 429 e carrega retryAfterSeconds', () => {
    const err = new RateLimitedError(45);
    expect(err.status).toBe(429);
    expect(err.type).toBe('https://errors.example.com/auth/rate-limited');
