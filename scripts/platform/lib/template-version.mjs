...
export function compareSemver(a, b) {
  ...
  if (!left || !right)
    throw new TypeError(`tag fora do padrão vX.Y.Z: ${!left ? a : b}`)
  return (
...
export function readTemplateOrigin(answersPath) {
  ...
  return {
    ...
    commit: typeof answers._commit === "string" ? answers._commit : undefined,
  }
}
...
export function listRemoteStableTags(
  source,
  { timeoutMs = 8000, exec = execFileSync } = {}
) {
...
export function cachedRemoteStableTags(
  ...
  if (cachePath)
    writeFileSync(cachePath, JSON.stringify({ source, checkedAt: now, tags }))
  return tags
}
...
export function computeTemplateStatus({ commit, tags }) {
  ...
  if (!installed)
    return { installed: undefined, aheadBy: 0, latest, behind: tags }
  const behind = tags.filter((tag) => compareSemver(tag, installed.version) > 0)
  return {
    installed: installed.version,
    aheadBy: installed.aheadBy,
    latest,
    behind,
  }
}
...
export function buildTemplateBehindReport({
  status,
  pendingKernelAdvisories = [],
}) {
  const ahead =
    status.aheadBy > 0
      ? ` (installed from a commit ${status.aheadBy} past that tag)`
      : ""
  const lines = [
  ...
  for (const advisory of pendingKernelAdvisories) {
    ...
    lines.push(
      `${advisory.id} ${advisory.kind} ${advisory.severity} kernel — fix: ${firstFixLine}`
    )
  }
  return lines.join("\n")
}
