...
const DEFAULT_LOG = join(homedir(), ".claude", "projects", REPO_SLUG, "dispatch-log.jsonl");
const logPath = process.argv[2] ?? DEFAULT_LOG;

if (!existsSync(logPath)) {
  console.error(`No dispatch log at ${logPath} — the hook writes it from the first dispatch of a new session.`);
  process.exit(1);
}
...
const stops = rows.filter((r) => r.ev === "stop" && (r.type || r.transcript !== "missing"));
...
const dispatchByToolUse = new Map(dispatches.filter((d) => d.tool_use_id).map((d) => [`${d.session}:${d.tool_use_id}`, d]));
const toolUseByAgent = new Map(starts.filter((s) => s.agent_id && s.tool_use_id).map((s) => [`${s.session}:${s.agent_id}`, s.tool_use_id]));
...
const joined = stops.map((stop) => {
  ...
  let dispatch = toolUse ? dispatchByToolUse.get(`${stop.session}:${toolUse}`) : undefined;
...
const runKey = (j, i) => (j.stop.agent_id ? `${j.stop.session}:${j.stop.agent_id}` : `__row${i}`);
const resumesByAgent = new Map();
const runByAgent = new Map();
joined.forEach((j, i) => {
  const key = runKey(j, i);
  if (runByAgent.has(key)) resumesByAgent.set(key, (resumesByAgent.get(key) ?? 1) + 1);
  runByAgent.set(key, j);
});
const runs = [...runByAgent.values()];
const resumesOf = (j) => (j.stop.agent_id ? (resumesByAgent.get(`${j.stop.session}:${j.stop.agent_id}`) ?? 0) : 0);
...
console.log(`${dispatches.length} dispatches · ${starts.length} starts · ${stops.length} stops · ${splits.length} wave-split blocks\n`);
...
const typeLines = [...groups.entries()]
  ...
  .map(([key, js]) => {
    ...
    for (const j of js) outcomes[j.stop.outcome ?? "unknown"] = (outcomes[j.stop.outcome ?? "unknown"] ?? 0) + 1;
    return [
      type,
      tier,
      js.length,
...
table(["type", "tier", "runs", "resumes", "med turns", "med ctx", "p90 ctx", "$", "done/stopped/unknown"], typeLines);
...
const sessionLines = sessions.map((session) => {
  ...
  const waves = new Set(workers.map((d) => d.wave).filter((w) => w !== null && w !== undefined));
  const split = new Set(splits.filter((s) => s.session === session).map((s) => s.wave));
  const forks = own.filter((d) => d.type === "fork").length;
  const spent = runs.filter((j) => j.stop.session === session).reduce((s, j) => s + cost(j), 0);
  return [session.slice(0, 8), own.length, workers.length, waves.size, split.size, forks, usd(spent)];
});
console.log("\nPer session");
table(["session", "dispatches", "spec-workers", "waves", "waves split", "forks", "$"], sessionLines);
...
if (unknownTier) console.log(`${unknownTier} run(s) had no model in the transcript nor a tier in the dispatch — priced as opus (inherited tier = the caller's).`);
const missing = stops.filter((s) => s.transcript === "missing").length;
if (missing) console.log(`${missing} stop row(s) without a transcript on disk — usage fields skipped for those.`);
...
const transcriptPathOf = (j) => (j.stop.session && j.stop.agent_id ? join(projectsDir, j.stop.session, "subagents", `agent-${j.stop.agent_id}.jsonl`) : null);
...
const warmupOf = (transcriptPath) => {
  ...
  for (const line of lines) {
    ...
    if (row.message?.id && row.message.usage) turnsSeen.set(row.message.id, row.message.usage);
    const edit = content.find((b) => b.type === "tool_use" && (b.name === "Edit" || b.name === "Write") && CODE_EDIT_PATH_RE.test(String(b.input?.file_path ?? "")));
    if (!edit || !firstTs || !row.timestamp) continue;
    let ctx = 0;
    for (const usage of turnsSeen.values()) ctx += (usage.input_tokens ?? 0) + (usage.cache_read_input_tokens ?? 0) + (usage.cache_creation_input_tokens ?? 0);
    return { reached: true, turns: turnsSeen.size, ctx, minutes: (Date.parse(row.timestamp) - Date.parse(firstTs)) / 60000 };
  }
...
const med = (values, fmt) => (values.length ? fmt(quantile(values, 0.5)) : "n/a");
const warmupLines = [...warmupGroups.entries()]
  .sort(([a], [b]) => a.localeCompare(b))
  .map(([type, ws]) => {
    const reached = ws.filter((w) => w.reached);
    const neverPct = Math.round(((ws.length - reached.length) / ws.length) * 100);
    return [
      type,
      ws.length,
      med(
...
if (warmupLines.length) {
  ...
  table(["type", "n", "med turns", "med ctx", "med minutes", "never edited"], warmupLines);
}
