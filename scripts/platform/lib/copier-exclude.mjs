...
export function parseExcludeList(copierYamlContent) {
  ...
  return Array.isArray(doc._exclude)
    ? doc._exclude.filter((entry) => typeof entry === "string")
    : []
}
...
export function findExcludedImports({
  repoRoot,
  scriptsRoot = path.join(repoRoot, "scripts"),
  excludeList,
}) {
  const offenders = []
