...
const parseCluster = (prompt) => {
  const m =
    /\b[Cc]luster\s+([A-Z][\w-]*)/.exec(prompt) ?? /\b(C\d+)\b/.exec(prompt)
  return m ? m[1] : null
}
...
const waveStateFile = (session) =>
  path.join(tmpdir(), `platform-wave-${session}.json`)
...
const waveVerdict = (input, row) => {
  ...
  const sameMessage = Object.values(entry.clusters).filter(
    (ts) => ts - entry.first_ts <= WAVE_SAME_MESSAGE_S * 1000
  ).length
  const allowed =
    !state[key] ||
    ...
    (entry.blocked_at !== null &&
      now - entry.blocked_at < WAVE_BLOCK_GRACE_S * 1000)
  if (allowed) {
  ...
  const remaining = [
    row.cluster,
    ...row.siblings.filter(
      (c) => c !== row.cluster && entry.clusters[c] === undefined
    ),
  ]
  return {
    split: {
...
const findTranscript = (input, projectsDir) => {
  ...
  if (
    typeof own === "string" &&
    /[/\\]subagents[/\\]agent-[^/\\]+\.jsonl$/.test(own) &&
    existsSync(own)
  )
    return own
  return null
...
const readMeta = (transcriptPath) => {
  try {
    return JSON.parse(
      readFileSync(transcriptPath.replace(/\.jsonl$/, ".meta.json"), "utf8")
    )
  } catch {
    return {}
  }
...
const inheritedUuids = (transcriptPath, meta, projectsDir, session) => {
  const parentPath = meta.parentAgentId
    ? path.join(
        ...
        `agent-${meta.parentAgentId}.jsonl`
      )
    : projectsDir && session
  ...
  try {
    ...
    const set = new Set()
    for (const m of readFileSync(parentPath, "utf8").matchAll(
      /"uuid":"([0-9a-f-]{36})"/g
    ))
      set.add(m[1])
    return set
...
const commitHash = (text) => {
  const m =
    /\[[^\]\s]+ ([0-9a-f]{7,40})\]/.exec(text) ??
    /^\s*([0-9a-f]{7,40})\s+\S/.exec(text)
  return m ? m[1] : null
}
...
const analyse = (transcriptPath, skip) => {
  ...
  for (const line of readFileSync(transcriptPath, "utf8").split("\n")) {
    ...
    if (row.type === "assistant") {
      if (row.message?.id && row.message.usage)
        turns.set(row.message.id, row.message.usage)
      ...
      for (const block of content) {
        ...
        if (
          block.type === "tool_use" &&
          block.name === "Bash" &&
          /\bgit\s+commit\b/.test(String(block.input?.command ?? ""))
        ) {
      ...
      for (const block of content) {
        if (block.type !== "tool_result" || !commitCalls.has(block.tool_use_id))
          continue
  ...
  const durationMs =
    first && last ? Date.parse(last) - Date.parse(first) : Number.NaN
  return {
    model,
    turns: turns.size,
    ctx,
    ...
    duration_s: Number.isFinite(durationMs)
...
const stopRow = (input, projectsDir) => {
  ...
  if (!transcriptPath) {
    ...
    return {
      ...
      model: null,
      transcript: "missing",
      outcome: classifyOutcome(lastGiven),
      last: lastGiven,
    }
  }
  ...
  const skip = fork
    ? inheritedUuids(transcriptPath, meta, projectsDir, input.session_id)
    : null
  ...
  if (copy.tool_input && typeof copy.tool_input === "object") {
    ...
    copy.tool_input = {
      ...
      ...(typeof prompt === "string"
        ? { prompt: prompt.slice(0, DEBUG_PROMPT_CHARS) }
        : {}),
    }
  }
  return {
...
try {
  ...
  const file = projectsDir
    ? path.join(projectsDir, "dispatch-log.jsonl")
    : path.join(homedir(), ".claude", "platform-dispatch-log.jsonl")
  if (process.env.PLATFORM_DISPATCH_DEBUG === "1")
    append(file.replace(/[^/\\]+$/, "dispatch-debug.jsonl"), debugRow(input))
