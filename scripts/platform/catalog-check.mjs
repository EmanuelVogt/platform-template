  ...
  return {
    wrap(run, { defaultMs, contractMs }) {
      return (command, args = [], options = {}) => {
        ...
        const timeoutMs =
          options.timeoutMs ?? (isContract ? contractMs : defaultMs)
        const result = run(command, args, { ...options, timeoutMs })
        if (result.timedOut) {
          last = {
            stepLabel: isContract ? "contract" : [command, ...args].join(" "),
            timeoutMs,
          }
        }
        return result
...
function timeoutFailure(log, stepLabel, timeoutMs) {
  log(
    `catalog:check — passo "${stepLabel}" excedeu o tempo limite (${timeoutMs}ms); processo encerrado`
  )
...
export async function runCatalogCheck({
  ...
  try {
    ...
    if (err instanceof CyclicDependencyError) {
      log(
        `catalog:check — ciclo de dependências detectado: ${err.chain.join(" -> ")}`
      )
  ...
  try {
    ...
    const timedRun = timeoutTracker.wrap(run, {
      defaultMs: stepTimeoutMs,
      contractMs: contractTimeoutMs,
    })

    log(`catalog:check — renderizando child kernel-only em ${childDir}`)
    const renderResult = renderChild({
      repoRoot,
      targetDir: childDir,
      run: timedRun,
    })
    ...
    if (renderResult.status !== 0) {
      log(
        `catalog:check — falha ao renderizar o child (copier saiu com código ${renderResult.status})`
      )
      return EXIT_CODES.CATALOG_UNREACHABLE
    }
    ...
    if (existsSync(answersPath)) {
      ...
      try {
        ...
        log(
          `catalog:check — não foi possível determinar a versão do kernel a simular: ${err.message}`
        )
        return EXIT_CODES.USAGE_ERROR
      }
      log(
      ...
      writeSimulatedKernelVersion({
        answersPath,
        kernelVersion: simulatedVersion,
      })
    }
    ...
    if (installResult.timedOut)
      return timeoutFailure(log, "pnpm install", stepTimeoutMs)
    if (installResult.status !== 0) {
      log(
        `catalog:check — falha ao instalar dependências do child (pnpm install saiu com código ${installResult.status})`
      )
      return EXIT_CODES.CATALOG_UNREACHABLE
    }
    ...
    if (lintResult.timedOut)
      return timeoutFailure(log, "pnpm catalog:lint", stepTimeoutMs)
    ...
    if (!gate.ok) {
      if (gate.result.timedOut)
        return timeoutFailure(log, gate.step, stepTimeoutMs)
      log(
        `catalog:check — "${gate.step}" falhou no child (código ${gate.result.status})`
      )
      return EXIT_CODES.TEST_FAILURE
    }
