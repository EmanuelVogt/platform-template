...
// Reasoning, rejected alternatives and history belong in the platform's decision
// log (`.ca-plans/`, a lean one-liner per decision) or a run's own design doc —
// never inline here.
// Three checks on the text a write ADDS:
//   1. growth — an edit may add at most MAX_GROWTH lines net; a new file may
//      have at most MAX_NEW_FILE lines;
//   2. rationale prose in a handbook — "por quê", "porque", "descartado",
...
if (tool === "Bash") {
  ...
  block(
    `Shell write into a handbook blocked — \`${cmd.slice(0, 100)}${cmd.length > 100 ? "…" : ""}\`.\nEdit docs with the Edit/Write tools so the text goes through the lean-docs checks (growth cap ${MAX_GROWTH} lines, no rationale prose in a handbook).`
  )
}
...
if (tool === "Write") {
  ...
  if (existsSync(filePath)) {
    ...
    if (lines > MAX_NEW_FILE) {
      block(
        `New handbook \`${filePath}\` has ${lines} lines (cap ${MAX_NEW_FILE}).\nA handbook is mechanism + commands + the caveat that bites. Cut, or split what is reasoning into the decision log (\`.ca-plans/\`) and link it.`
      )
    }
    newText = content
  }
...
if (added > MAX_GROWTH) {
  block(
    `${label}.\nA handbook is mechanism + commands + the caveat that bites. Cut to the lines a reader needs to act; reasoning, history and rejected alternatives go to the decision log \`.ca-plans/\` (link it, don't restate it).`
  )
}
...
if (heading || phrase) {
  ...
  block(
    `Rationale prose in a handbook blocked — "${hit}" in \`${filePath}\`.\nHandbooks say what to do and how; the why, the rejected alternatives and the history live only in the decision log \`.ca-plans/\` or a run's own design doc. Link it and delete the explanation.`
  )
}
