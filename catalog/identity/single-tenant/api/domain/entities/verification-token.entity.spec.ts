...
function buildToken(overrides: Partial<Parameters<typeof VerificationToken.create>[0]> = {}) {
...
describe('VerificationToken.isValid (clock injetado)', () => {
  ...
  it('borda: expiresAt == now é inválido (> estrito)', () => {
    const token = VerificationToken.fromProps({ ...buildToken().props, expiresAt: new Date(NOW) });
    expect(token.isValid(NOW)).toBe(false);
  });
