...
describe("DrizzleAuditRepository (int)", () => {
  ...
  it("filtra por janela from/to (occurred_at)", async () => {
    ...
    await seed({
      occurredAt: new Date("2026-12-01T00:00:00Z"),
      table: "services",
      entityId: "new",
    })
    const res = await repo.list({
  ...
  it("q busca no conteúdo das alterações (row_new)", async () => {
    await seed({
      ...
      table: "services",
      entityId: "x",
      rowNew: { name: "Massoterapia" },
    })
    await seed({
      occurredAt: new Date(),
      table: "services",
      entityId: "y",
      rowNew: { name: "Fisioterapia" },
    })
