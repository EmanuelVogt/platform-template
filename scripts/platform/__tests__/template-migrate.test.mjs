...
import { discoverMigrationScripts, templateMigrateCommand } from "../lib/commands/template-migrate.mjs";
...
function makeChild({ commit = "v2.8.0" } = {}) {
  ...
  writeFileSync(path.join(dir, ".copier-answers.yml"), `_src_path: gh:acme/platform-template\n_commit: ${commit}\n`, "utf8");
  return dir;
}
...
test("roda scripts v2.9.0 e v3.0.0 em ordem ascendente até --target", async () => {
  ...
  const exit = await templateMigrateCommand({ options: { target: "v3.0.0" }, cwd: dir, log: () => {} });
...
test("re-execução de uma migração já aplicada é no-op", async () => {
  ...
  await templateMigrateCommand({ options: { target: "v3.0.0" }, cwd: dir, log: () => {} });
  const firstRun = readFileSync(path.join(dir, "order.log"), "utf8");

  const secondExit = await templateMigrateCommand({ options: { target: "v3.0.0" }, cwd: dir, log: () => {} });
...
test("script que falha para a execução e nomeia o script; os seguintes não rodam", async () => {
  ...
  let exit;
  try {
    exit = await templateMigrateCommand({ options: { target: "v3.0.0" }, cwd: dir, log: () => {} });
  } finally {
...
test("sem diretório de migrações: no-op com sucesso", async () => {
  ...
  assert.equal(exit, EXIT_CODES.OK);
  assert.deepEqual(discoverMigrationScripts(path.join(dir, "scripts", "platform", "migrations")), []);
});
...
test("registrado no cli: run(['template', 'migrate', ...]) chega ao comando", async () => {
  ...
  const exit = await run(["template", "migrate", "--target", "v2.9.0"], { cwd: dir });
