...
test("runTemplateSmoke invoca pnpm platform status e pnpm platform list dentro do próprio child renderizado", async () => {
  ...
  const statusCall = run.calls.find((c) => c.command === "pnpm" && c.args.join(" ") === "platform status");
  const listCall = run.calls.find((c) => c.command === "pnpm" && c.args.join(" ") === "platform list");
...
test("uma reintrodução deliberada do import excluído (a CLI crasha no child) deixa o smoke vermelho", async () => {
  ...
  const run = stubRun(greenOverrides({ "platform status": { status: 1, stdout: "", stderr: "Cannot find module" } }));

  const code = await runGreenSmoke(run, childDir);

  assert.equal(code, EXIT_CODES.TEST_FAILURE);
  const listCall = run.calls.find((c) => c.command === "pnpm" && c.args.join(" ") === "platform list");
  assert.equal(listCall, undefined, "não deveria seguir para list depois que status falhou");
});
