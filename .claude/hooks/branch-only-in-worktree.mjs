...
const dirKind = (dir) => {
  ...
  for (;;) {
    try {
      return statSync(path.join(current, '.git')).isFile() ? 'worktree' : 'main-checkout';
    } catch {
...
const createsBranch = (tokens, gitIdx) => {
  ...
  let creates = false;
  if (sub === 'checkout') {
    creates = rest.some((t) => t === '-b' || t === '-B' || t === '--orphan' || /^-[bB]./.test(t));
  } else if (sub === 'switch') {
    creates = rest.some(
      (t) =>
        t === '-c' || t === '-C' || t === '--create' || t === '--force-create' ||
        t === '--orphan' || /^-[cC]./.test(t),
    );
  } else if (sub === 'branch') {
    const hasNonCreating = rest.some((t) => BRANCH_NON_CREATING.has(t.split('=')[0]));
