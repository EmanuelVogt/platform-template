...
const HOOK_PATH = path.join(REPO_ROOT, ".claude", "hooks", "template-behind.mjs");
...
function seedTagsCache(source, tags) {
  ...
  const cachePath = path.join(tmpdir(), `platform-template-tags-${cacheKey}.json`);
  writeFileSync(cachePath, JSON.stringify({ source, checkedAt: Date.now(), tags }));
}

test("template repo itself (no _src_path): silent, no feed fetched (FEED-04)", () => {
  const projectDir = mkdtempSync(path.join(tmpdir(), "template-behind-notemplate-"));
...
test("behind + pending: the advisory published only at the latest tag is printed (FEED-01/02)", () => {
  ...
  assert.equal(context.split("\n")[0], "template behind: installed v1.0.0, latest v2.0.0 — 1 tag(s): v2.0.0");
  assert.equal(context.split("\n")[2], "ADV-20260821-01 bug high kernel — fix: copier update to >= v2.0.0");
});
...
test("cached feed still prints its advisory once the source is gone (FEED-03 cache-only)", () => {
  ...
  assert.match(first, /^ADV-20260821-01/m, "run 1 must have populated both caches");

  rmSync(source, { recursive: true, force: true });

  const second = additionalContextOf(runHook({ projectDir }));
  assert.equal(second, first, "run 2 reads from cache alone, the source is unreachable");
});
