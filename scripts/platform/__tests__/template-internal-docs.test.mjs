...
test("no tracked file under the internal dir is a handbook the product would look for", () => {
  const tracked = execFileSync("git", ["ls-files", "-z", INTERNAL_DIR], { cwd: ROOT, encoding: "utf8" })
    .split("\0")
    .filter(Boolean);
  const jinja = tracked.filter((file) => file.endsWith(".jinja"));
  assert.deepEqual(jinja, [], "an internal analysis is never rendered — drop the .jinja suffix");
});
