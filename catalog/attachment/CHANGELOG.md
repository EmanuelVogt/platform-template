...
- `README.md`: § Paridade passa a dizer onde vivem os helpers de teste da entrada
  (`api/testing/` — `inMemoryStorage`, `makeAttachment`, `PNG_1PX`, `seedAttachment`) e quem os
  importa hoje (ninguém; o e2e desta entrada consome `identity/api/testing/` para sessão). Sem
  mudança de código; a versão sobe porque REL-04 exige que qualquer mudança no diretório da
  entrada desde a tag anterior mova a versão, inclusive uma mudança só de documentação.
