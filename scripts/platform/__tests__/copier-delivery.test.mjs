...
test("no self-updating bot ships to the child — the bot files do not exist", () => {
  const tracked = execFileSync(
    ...
    [
      ...
      "scripts/platform/template-update-ci.mjs",
    ],
...
test("scripts/platform/migrations ships to the child (no tracked file excluded)", () => {
  const tracked = execFileSync(
    "git",
    ["ls-files", "-z", "scripts/platform/migrations"],
    {
      cwd: ROOT,
      encoding: "utf8",
    }
  ...
  assert.ok(
    tracked.length > 0,
    "expected tracked files under scripts/platform/migrations by wave 2"
  )
  const list = excludes()
  for (const file of tracked) {
    assert.ok(
      !list.includes(file),
      `${file} must not be in copier _exclude — migrations ship to the child`
    )
  }
  assert.ok(
    !list.includes("scripts/platform/migrations"),
    "the whole migrations dir must not be excluded"
  )
  assert.ok(
    !list.includes("/scripts/platform/migrations"),
    "the whole migrations dir must not be excluded"
