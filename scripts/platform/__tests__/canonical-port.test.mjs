...
test("exactly one API port literal appears across every site a child reads", () => {
  const sites = [
    ["apps/api/src/shared/config/env.ts", /PORT: z\.coerce\.number\(\)\.int\(\)\.positive\(\)\.default\((\d+)\)/],
  ...
  for (const [site, value] of ports) {
    assert.equal(value, canonical, `${site} carries port ${value}, expected ${canonical} — drifted from the other sites`);
  }
});

test("env.ts default, apps/api/.env.example and local-environment.md agree on 3000", () => {
  assert.equal(extractPort("apps/api/src/shared/config/env.ts", /PORT: z\.coerce\.number\(\)\.int\(\)\.positive\(\)\.default\((\d+)\)/), "3000");
  assert.equal(extractPort("apps/api/.env.example", /^PORT=(\d+)$/m), "3000");
  assert.equal(extractPort("docs/dev/local-environment.md", /api \((\d+)\)/), "3000");
});
...
test("docker-compose.yml api service: host mapping and container port agree", () => {
  ...
  assert.equal(hostPort, containerPort, "host and container ports disagree in the api service mapping");
