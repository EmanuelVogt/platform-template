...
if (!/\.specs\/.*\/tasks\.md$/.test(abs.split(path.sep).join("/")))
  process.exit(0)
...
const isPathLike = (s) =>
  !/\s/.test(s) && (s.includes("/") || /\w\.\w+$/.test(s))
...
for (const [wave, siblings] of [...byWave.entries()].sort(
  (a, b) => a[0] - b[0]
)) {
  ...
  const solo = siblings.filter(
    (c) => c.taskIds.length === 1 && !tasks.get(c.taskIds[0])?.exclusive
  )
  if (solo.length >= 3) {
    violations.push(
  ...
  const waveTasks = siblings.flatMap((c) =>
    c.taskIds.map((id) => ({ id, label: c.label }))
  )
  for (const { id, label } of waveTasks) {
    if (!tasks.get(id)?.exclusive || waveTasks.length === 1) continue
    const others = waveTasks
      .filter((t) => t.id !== id)
      .map((t) => `${t.id} (${t.label})`)
      .join(", ")
    violations.push(
