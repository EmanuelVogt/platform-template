...
const IS_MAIN_PATH = path.join(REPO_ROOT, "scripts", "platform", "lib", "is-main.mjs");
...
test("isMain compara pelo pathToFileURL do argv[1], não por concatenação de string", () => {
  ...
  assert.equal(isMain(pathToFileURL(argv1).href, "/repo/scripts/platform/other.mjs"), false);
  assert.equal(isMain("file:///repo/scripts/platform/cli.mjs", undefined), false);
...
test("nenhum arquivo sob scripts/** ainda compara import.meta.url por concatenação crua", () => {
  const offenders = walkScriptFiles(path.join(REPO_ROOT, "scripts")).filter((file) => RAW_GUARD_RE.test(readFileSync(file, "utf8")));
