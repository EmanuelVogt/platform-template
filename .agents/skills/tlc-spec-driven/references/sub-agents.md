...
2026-08-21 over 43 workers) plus a payload, a summary and the orchestrator's turns around it, to
...
3 000 lines of jest output into itself loses the task — but so does a worker that spends four turns
dispatching a runner, re-reading its answer and re-dispatching it at a higher tier (measured
2026-08-21: 73 of 196 worker→runner dispatches had to escalate to sonnet just to slice a red log).
So the worker and the Verifier **run their own gates directly, with the log on disk** —
`LOG=$(mktemp -t platform-run).log; <cmd> > "$LOG" 2>&1; echo exit=$?`, then `grep -n`/`tail -n 80`
...
| Scout | `Agent(subagent_type: "repo-scout", model: "haiku"\|"sonnet")` — haiku pointed question, sonnet module map; `model` required, the dispatch itself optional inside a worker | the `repo-discovery` skill, run by the worker itself |
| Runner | `Agent(subagent_type: "shell-runner", model: "haiku")` — main window's Build gate and the Verifier's Final gate only; `model` required | the same two gates redirected to a file by whoever runs them |
...
The Claude Code hook `delegate-to-subagent.mjs` **enforces** the discipline on the main thread
(two navigations per turn, zero heavy runs). Inside `spec-worker` and `spec-verifier` it counts
neither navigation nor gate runs — they run their own commands — and keeps only the **Read byte
budget** for the agent's lifetime, the guard against a worker reading a 30 kB reference whole
before it edits anything; it stays silent inside scout and runner.
`subagent-model-required.mjs` enforces the explicit `model` on every dispatch of the four agents,
in the main window and inside nesting subagents alike. In other harnesses the same rules are text
in the worker payload.
...
- **Cluster** — an **ordered** list of tasks handed to **one worker**, executed sequentially: a
  ...
  warm-up before it edits anything (measured 2026-08-21 over 43 workers: a median 21 turns and 94k
...
6. **Re-derive on change** — the levels are only as good as the `Depends on`/`Touches` they were
   ...
   `serial-ok: FIFO tail`) instead of waiting for a barrier it no longer needs. Measured
   2026-08-20: one recomputation removed a wave and surfaced two plan gaps one wave earlier.
...
clusters. Measured 2026-08-21 on one feature's waves 2–11: 44 of 45 clusters held a single task, so
...
- **≥3 single-task non-exclusive clusters in one wave is an authoring smell**: the vertical was cut
  ...
  slice. Merge them into the slice's cluster (wiring last) before presenting. In Claude Code
  `wave-plan-check.mjs` warns on it at every write of `tasks.md`.
...
- **Gates go through the runner here**, unlike inside a worker: this is the planning window, where
  a heavy command is delegated (`shell-runner`, haiku) so the log never lands in the session's most
  expensive context.
- **Why inline at this size:** a worker pays a full warm-up before its first edit (measured
  2026-08-21: a median 21 turns and 94k of context over 43 workers), plus a payload, a summary and
...
- **The Verifier still runs, always** — a fresh `spec-verifier`, sonnet by default, opus only per
...
2. **Dispatch the wave:** one worker per cluster, **all in a single message**, each with the worker
   ...
   queue the rest FIFO. In Claude Code this is enforced by `dispatch-log.mjs`: a second cluster
   of the same wave more than 2 min after the first is blocked unless 4 are already in flight or
   the payload says `serial-ok: <reason>` (measured 2026-08-20: 16 of 17 multi-cluster waves
   went out one worker at a time).
...
5. **Scoped gates only, run by you, logged to a file.**
   `LOG=$(mktemp -t platform-run).log; <cmd> > "$LOG" 2>&1; echo exit=$?`, then `grep -n` the
...
| Worker STOPPED `blocked-by-ownership` | The plan is wrong, not the worker: add the file to that task's `Touches`. File owned by nobody in flight → **resume the same worker** (`SendMessage`) with the expanded grant and the siblings restated — it already holds the task's context (measured 2026-08-20: four resumptions, four completions, no re-read of card/plan/module). File owned by a sibling in flight → wait for that sibling, then resume the stopped worker (same cluster label). Correct `tasks.md` at wave close so the cross-check holds again. Second `blocked-by-ownership` in the same feature → stop dispatching, run the Touches audit (tasks.md § 3, *Layer completeness* included) over every cluster not yet dispatched, and re-derive the levels (§ *Waves and clusters* step 6); the gap repeats otherwise (measured: the same cluster stopped twice on two different unlisted files). |
