...
const transcripts = () => {
  ...
  return files.sort((a, b) => statSync(a).mtimeMs - statSync(b).mtimeMs).slice(-SESSIONS);
};
...
const texto = (content) => (typeof content === "string" ? content : JSON.stringify(content ?? ""));
...
for (const file of transcripts()) {
  ...
  if (turnosSessao) sessoes.push({ file, turnos: turnosSessao, base, pico, leitura });
}
...
console.log(`  cache read   ${uso.cacheRead.toLocaleString("pt-BR")}  <- o custo dominante`);
...
console.log(`  piso de sessão nova (mediana do 1º turno): ${k(bases[Math.floor(bases.length / 2)] ?? 0)}`);
...
const navegacao = (bashChamadas.get("busca (grep/find/ls)") ?? 0) + (bashChamadas.get("leitura via shell (cat/sed/head)") ?? 0);
console.log(
