...
import { compareSemver, parseInstalledVersion, parseSemverTag, readTemplateOrigin } from "../template-version.mjs";
...
function resolveTarget({ options, cwd }) {
  ...
  if (!installed) {
    return { error: "não foi possível determinar a versão-alvo — passe --target vX.Y.Z" };
  }
...
export async function templateMigrateCommand({ options = {}, cwd = process.cwd(), log = defaultLog } = {}) {
  ...
  const due = scripts.filter((script) => compareSemver(script.version, target) <= 0);
  for (const script of due) {
    ...
    try {
      ...
      process.stderr.write(`migração ${script.version} falhou: ${err.message}\n`);
