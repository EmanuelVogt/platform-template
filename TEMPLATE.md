...
`release.yml` after the full gate — never locally — and the same job publishes the GitHub
Release, taking that version's changelog section as its notes. The product updates with
`copier update` (see `docs/dev/template.md`).
