...
49. **The RULE C token list is narrower than its intent.** It catches `auditTrail`/`audit_trail`/`AuditRegistry` but not the PascalCase, SCREAMING_SNAKE or kebab forms of the same concept, so `AuditTrailPurger` and `AUDIT_TRAIL_PURGER` entered the kernel without tripping it. Either the list gains those forms — and then AD-024's ports need module-agnostic names — or the intent is genuinely narrower than it reads. **Resolved at the Verifier (2026-08-21): the intent is the narrower one.** RULE C guards the listed vocabulary tokens as a tripwire; kernel-owned maintenance jobs and ports introduced by AD-022/AD-024 (`AuditTrailPurger` & co.) are module-agnostic runtime concepts reviewed by name, not by regex. Widening to PascalCase/SCREAMING_SNAKE forms would force renames of kernel jobs for no boundary gain; if a future entry needs to *replace* such a job, that is the moment to rename and widen.
...
### Wave 7 — Verifier fixes (2026-08-21; `validation.md` round 1 = FAIL)

| Task | Commit | What |
| --- | --- | --- |
| T29 | `4b7445e` | `template-smoke.mjs` schema check is a forbidden-extra check over `{_kernel, drizzle, public}` (Fix 1a; spec KRN-01 AC3 corrected) |
| T30 | `f1849f2` | `catalog-custom-migrations.test.mjs`: `dependsOn` re-derived from production imports of every entry and compared to `module.json`; mutant `attachment.dependsOn: []` killed (Fix 3) |
| T31 | `f59651a` `011a035` | `.github/workflows/catalog.yml`: `gates` (check/test/test:scripts/catalog:lint/catalog:typecheck + ADV-04 on PRs), `catalog` matrix `[identity, attachment, audit, notification, tag]` with postgres/redis services, `smoke`; triggers PR/push main/tags `v*`; excluded from copier (`_exclude`). **Not executed** (no Actions here). ADV-04 step simulates the PR diff via `git reset --soft <base>` because `advisory-required.mjs` only has a commit-msg mode — judgment call, review (Fix 2) |
| T32 | `0fbfb44` | `back-arch.md` RULE B → RULE C (16 tokens); changelog v1.0.0 `RouteAccess` line (Fix 4, note 27) |
| T33 | `b2a378a` | `docs/advisories/{APPLIED.md,README.md}`; `pending-advisories.test.mjs` covers the 4 ADV-02 branches; hook no-lock string aligned to spec.md:251 (design.md:222 still says `pnpm platform module adopt` — align design) (Fix 5) |
| T34 | `5027c21` | `loadAdvisories`/`catalog-lint` only read `ADV-YYYYMMDD-NN.md` (`isAdvisoryFilename`) — before, README/APPLIED made `catalog:lint` exit 1 and the hook silently swallow; hook emits spec.md:190 line format without prefix |
| T35 | `e42ab2a` | `template:smoke` keeps Postgres alive across checks, starts ephemeral Redis, passes `DATABASE_URL`/`REDIS_URL`/`WEB_ORIGIN`/`R2_*` to the child api, surfaces child stderr on timeout. **First real green run: 4/4, twice, no leaked containers** |

Counts at `e42ab2a`: `test:scripts` 176, `catalog:lint` 0, `catalog:typecheck` 0, `template:smoke` exit 0. `catalog:check` not re-run after wave 7 (only `scripts/**`, docs, CI, hook changed — catalog sources untouched since round 13). Fix 6 (TLG assertion gaps, MINOR) deliberately not done — follow-up. Fix 7 spec corrections applied on main (KRN-01 AC3 `public`; P1-ports AC3 outbox dropped; cross-module table = AD-025 graph; CAT-01 AC1 no `files`; note 49 resolved as "intent is narrower").

71. **A smoke that was never run is not a gate.** `template:smoke` shipped in wave 3 and was first executed by the Verifier: two independent defects (schema equality, child env/infra lifecycle) and one more found only because stderr became visible (R2 env). Any script whose job is "boot a rendered child" must be run for real before its task is marked done — `catalog:check` learned this in wave 6 (13 rounds), `template:smoke` repeated it.
72. **Loaders over a docs folder must filter by the id shape.** `docs/advisories/` legitimately holds `README.md`/`APPLIED.md`; a glob over `*.md` turned both into parse errors that one consumer surfaced (`catalog:lint`) and the other swallowed (hook). Found only when T33 created the folder the spec required from the start.
