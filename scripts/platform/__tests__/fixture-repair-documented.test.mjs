...
test("no .copier-answers.yml (leading dot) is tracked — the fixture stays copier-answers.yml", () => {
  const tracked = execFileSync("git", ["ls-files", "-z"], { cwd: REPO_ROOT, encoding: "utf8" })
    .split("\0")
    .filter(Boolean);
  const leaked = tracked.filter((file) => path.basename(file) === ".copier-answers.yml");
  assert.deepEqual(leaked, []);
  assert.ok(tracked.includes("scripts/platform/__tests__/fixtures/child/copier-answers.yml"));
});
