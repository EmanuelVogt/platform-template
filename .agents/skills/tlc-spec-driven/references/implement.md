...
2026-08-17 over 339 sub-agent runs: ≈990 tokens of own output per turn (35 % thinking) against
...
If the Verifier returns FAIL, the orchestrator turns the ranked gaps into fix tasks (clustered and dispatched to workers like any other wave), then re-verifies by **resuming the same Verifier** (`sub-agents.md` § *Failure handling*: `SendMessage` with the fix range and the gaps addressed, while it is under its turn budget; fresh only when over budget) — bounded to a max of **3 fix→re-verify iterations** before escalating to the user.
