...
describe("attachment.config", () => {
  ...
  it("coage valores de env", () => {
    expect(
      parseAttachmentConfig({ ATTACHMENT_MAX_UPLOAD_BYTES: "100" })
        .ATTACHMENT_MAX_UPLOAD_BYTES
    ).toBe(100)
    expect(
      parseAttachmentConfig({ ATTACHMENT_MULTI_MAX_FILE_BYTES: "2048" })
        .ATTACHMENT_MULTI_MAX_FILE_BYTES
    ).toBe(2048)
    expect(
      parseAttachmentConfig({ ATTACHMENT_PENDING_QUOTA_BYTES: "1000" })
        .ATTACHMENT_PENDING_QUOTA_BYTES
    ).toBe(1000)
    expect(
      parseAttachmentConfig({ ATTACHMENT_MAX_CONCURRENT_UPLOADS: "4" })
        .ATTACHMENT_MAX_CONCURRENT_UPLOADS
    ).toBe(4)
  })

  it("não conhece mais os envs antigos de feedback/report", () => {
    expect(attachmentConfigSchema.shape).not.toHaveProperty(
      "ATTACHMENT_FEEDBACK_MAX_FILE_BYTES"
    )
    expect(attachmentConfigSchema.shape).not.toHaveProperty(
      "ATTACHMENT_FEEDBACK_MAX_TOTAL_BYTES"
    )
    expect(attachmentConfigSchema.shape).not.toHaveProperty(
