...
import { assertWebShell, InvalidWebStackError, parseWebStack, WebShellMismatchError } from "../lib/web-shell.mjs";

function tmpChild() {
  return mkdtempSync(path.join(tmpdir(), "web-shell-fixture-"));
}

function writeWebShell(childDir, { name = "web", configFile, extraFiles = {} } = {}) {
  const webDir = path.join(childDir, "apps", "web");
  mkdirSync(webDir, { recursive: true });
  writeFileSync(path.join(webDir, "package.json"), JSON.stringify({ name }));
  if (configFile) writeFileSync(path.join(webDir, configFile), "export default {}\n");
...
test("parseWebStack rejects any other value with a pt-BR message", () => {
  assert.throws(
    ...
    (err) => err instanceof InvalidWebStackError && err.value === "svelte" && /vite.*next|next.*vite/.test(err.message),
  );
});
...
test("assertWebShell accepts a well-formed vite child (happy path)", () => {
  const childDir = tmpChild();
  try {
    writeWebShell(childDir, { configFile: "vite.config.ts", extraFiles: { "src/main.tsx": 'from "react"\n' } });
...
test("assertWebShell accepts a well-formed next child (happy path)", () => {
  const childDir = tmpChild();
  try {
    writeWebShell(childDir, { configFile: "next.config.ts", extraFiles: { "app/page.tsx": "export default function Page() {}\n" } });
    assert.doesNotThrow(() => assertWebShell(childDir, "next"));
  } finally {
    rmSync(childDir, { recursive: true, force: true });
  }
...
test("assertWebShell rejects a shape mismatch (vite.config.ts present when --web-stack next was requested)", () => {
  ...
  try {
    ...
    assert.throws(
      ...
      (err) => err instanceof WebShellMismatchError && /formato esperado/.test(err.message),
    );
  } finally {
    rmSync(childDir, { recursive: true, force: true });
  }
...
test("assertWebShell rejects a Next child that still references VITE_/@tanstack/react-router/nginx (ACC-06)", () => {
  ...
  try {
    ...
    assert.throws(
      () => assertWebShell(childDir, "next"),
      (err) => err instanceof WebShellMismatchError && err.message.includes("VITE_"),
    );
  } finally {
    rmSync(childDir, { recursive: true, force: true });
  }
...
test("assertWebShell rejects a Vite child that imports from next (ACC-06)", () => {
  ...
  try {
    ...
    assert.throws(
      ...
      (err) => err instanceof WebShellMismatchError && err.message.includes('from "next'),
    );
  } finally {
    rmSync(childDir, { recursive: true, force: true });
  }
