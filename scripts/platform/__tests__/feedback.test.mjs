...
import {
  ...
  feedbackCommand,
  githubRepoOf,
} from "../lib/commands/feedback.mjs"
import { EXIT_CODES } from "../lib/exit-codes.mjs"

function makeChild({
  withAnswers = true,
  withLock = true,
  source = "gh:acme/platform-template",
} = {}) {
...
test("rascunho válido: compõe corpo carimbado e imprime gh + URL", async () => {
  ...
  assert.match(
    output,
    /gh issue create --repo acme\/platform-template --title 'Outbox retry loses the actor context'/
  )
  assert.match(
    output,
    /https:\/\/github\.com\/acme\/platform-template\/issues\/new\?title=/
  )

  const issue = readFileSync(
    path.join(dir, ".platform-feedback", "draft.issue.md"),
    "utf8"
  )
...
test("registrado no cli: run(['feedback', ...]) chega ao comando", async () => {
  const dir = makeChild()
  const draft = writeDraft(dir)
  const { result } = await captureOutput("stdout", () =>
    run(["feedback", draft], { cwd: dir })
  )
  assert.equal(result, EXIT_CODES.OK)
})
...
test("--json: estrutura estável para o agente", async () => {
  ...
  assert.match(
    parsed.ghCommand,
    /gh issue create --repo acme\/platform-template/
  )
  assert.match(parsed.url, /issues\/new\?title=/)
  assert.equal(
    parsed.outFile,
    path.join(".platform-feedback", "draft.issue.md")
  )
})
...
test("caminho de negócio do produto é bloqueado", async () => {
  const dir = makeChild()
  const draft = writeDraft(dir, {
    paths: ["apps/api/src/modules/billing/billing.service.ts"],
  })
  ...
  assert.match(
    output,
    /fora do escopo da plataforma: apps\/api\/src\/modules\/billing/
  )
  assert.equal(
    existsSync(path.join(dir, ".platform-feedback", "draft.issue.md")),
    false
  )
})
...
test("entrada instalada no lock passa; area catalog/<entry> sem lock é bloqueada", async () => {
  ...
  const ok = await captureOutput("stdout", () =>
    feedbackCommand({ draftPath: installed, cwd: dir })
  )
  ...
  const blocked = await captureOutput("stderr", () =>
    feedbackCommand({ draftPath: notInstalled, cwd: dir })
  )
...
test("frontmatter ausente ou incompleto é bloqueado", async () => {
  ...
  const missing = await captureOutput("stderr", () =>
    feedbackCommand({ draftPath: file, cwd: dir })
  )
  ...
  const typed = await captureOutput("stderr", () =>
    feedbackCommand({ draftPath: badType, cwd: dir })
  )
...
test("classifyPath espelha a tabela de ownership", () => {
  ...
  assert.equal(
    classifyPath(
      "apps/api/drizzle/migrations/0001_kernel_outbox_notify.sql",
      lock
    ).ok,
    true
  )
  assert.equal(
    classifyPath("apps/api/drizzle/migrations/1000_bookings.sql", lock).ok,
    false
  )
  assert.equal(
    classifyPath("apps/web/src/entities/tag/react/use-tags.ts", lock).ok,
    true
  )
...
test("githubRepoOf entende gh:, https e git@", () => {
  assert.equal(
    githubRepoOf("gh:acme/platform-template"),
    "acme/platform-template"
  )
  assert.equal(
    githubRepoOf("https://github.com/acme/platform-template.git"),
    "acme/platform-template"
  )
  assert.equal(
    githubRepoOf("git@github.com:acme/platform-template.git"),
    "acme/platform-template"
  )
