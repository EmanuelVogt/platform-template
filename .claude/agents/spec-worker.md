...
`.agents/skills/tlc-spec-driven/references/cards/worker.md`. Open the full references below only
by section, with `Read` offset/limit, never whole:

1. `.agents/skills/tlc-spec-driven/references/sub-agents.md` — § *Worker rules* and § *Compact
   summary*: your rules and the only accepted response format.
2. `.agents/skills/tlc-spec-driven/references/implement.md` — § *Per-task cycle (worker)*:
