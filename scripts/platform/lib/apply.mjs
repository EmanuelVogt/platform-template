...
function envBlockLines(moduleName, envVars, valueOf) {
  return [
    `# ${moduleName}`,
    ...envVars.map((envVar) => `${envVar.name}=${valueOf(envVar) ?? ""}`),
  ]
}
...
function appendEnvExampleBlock(filePath, moduleName, envVars) {
  ...
  appendLines(
    filePath,
    envBlockLines(moduleName, envVars, (envVar) => envVar.example)
  )
}
...
function renderPlatformModules(entries) {
  ...
  const sortedByPath = [...withApi].sort((a, b) =>
    a.apiModule.path.localeCompare(b.apiModule.path, undefined, {
      sensitivity: "base",
    })
  )
  const imports = sortedByPath
    .map(
      (entry) =>
        `import { ${entry.apiModule.export} } from "./${entry.apiModule.path}";`
    )
    .join("\n")
  const list = withApi
    .map((entry) => `resolvePlatformModule(${entry.apiModule.export})`)
    .join(", ")
