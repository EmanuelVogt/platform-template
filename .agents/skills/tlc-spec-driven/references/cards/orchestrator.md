...
the runner, one Build gate after the last task, then a fresh sonnet Verifier with a 1–2 mutant
...
re-cluster. **Touches audit** (tasks.md § 3) before the Wave Plan: one `repo-scout` (sonnet) call per
...
(+ port/repo if new) — `application/**` + `api/**` alone is not a grant. In Claude Code
`wave-plan-check.mjs` re-checks sibling overlap and exclusive-alone on every write of `tasks.md`.
...
One worker per cluster, **all clusters of the wave as `Agent` calls in a single message**, each with
the payload below and a tier chosen for that cluster. ≤4 in flight, rest FIFO. Hook-enforced
(`dispatch-log.mjs`): a second cluster of the same wave more than 2 min after the first is blocked
unless 4 are already in flight or the payload says `serial-ok: <reason>`. A continuation of a
STOPPED cluster keeps its label; a re-planned cluster gets the next wave number. **Tier per dispatch**:
sonnet default (CRUD, UI, tests, config, tooling, docs) · haiku only pure mechanics (payload says
"surgical edits, no formatter runs") · opus only domain entities/transitions, tx/outbox/ALS, migration,
...
| Signal | You do |
| --- | --- |
| `gate-failed` | Resume the same worker (`SendMessage`) with blocker + log path while under budget; fresh worker only on `HANDOFF:`. 2nd failure same tier → one tier up, fresh. 3 attempts → escalate. |
...
summaries before gating · edit `tasks.md` mid-wave · dispatch a `fork` or a placeholder to "wait"
(a finished sub-agent re-invokes you). Payloads, summaries and wave reports are English.
