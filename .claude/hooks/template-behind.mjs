...
import {
  ...
  mergeAdvisories,
} from "../../scripts/platform/lib/advisory-feed.mjs"
import {
...
try {
  ...
  try {
    ...
    const merged = mergeAdvisories(
      loadAdvisories(join(projectDir, "docs", "advisories")),
      feed.advisories
    )
    const ledger = readLedger(
      join(projectDir, "docs", "advisories", "APPLIED.md")
    )
    const { pending } = computePending(undefined, merged, ledger, {
      templateVersion: status.installed,
    })
  ...
  process.stdout.write(
    JSON.stringify({
      hookSpecificOutput: {
        hookEventName,
        additionalContext: buildTemplateBehindReport({
          status,
