...
function toEntry(value: unknown, position: number): JournalEntry {
  ...
  if (
    ...
    throw new Error(
      `entrada ${position} do journal não tem idx/when/tag válidos`
    )
  }
  return { idx, when, tag }
}
...
function readBaseEntries(): JournalEntry[] | null {
  try {
    ...
    const raw = execFileSync(
      "git",
      ["show", `${BASE_REF}:${JOURNAL_REPO_PATH}`],
      {
        encoding: "utf8",
        cwd: root,
        stdio: ["ignore", "pipe", "ignore"],
...
function checkFilePairing(entries: JournalEntry[], problems: string[]): void {
  ...
  for (const entry of entries) {
    if (!existsSync(join(MIGRATIONS_DIR, `${entry.tag}.sql`))) {
      problems.push(
        `${entry.tag}: está no journal mas o .sql não existe — o migrate quebra no boot`
      )
    }
  }
...
export function checkWhenOrdering(
...
export function detectBaselineReset(
  ...
  const droppedTags = base
    .filter((entry) => !branchTags.has(entry.tag))
    .map((entry) => entry.tag)
  const droppedBeyondBaseline = droppedTags.filter(
    (tag) => tag !== baseBaseline.tag
  )
...
export function checkAgainstBase(
  ...
  for (const entry of entries) {
    if (seen.has(entry.idx) && !baseDuplicateIdx.has(entry.idx)) {
      problems.push(
        `${entry.tag}: o número ${entry.idx} já é de outra migration — renumere antes do merge`
      )
    }
    seen.add(entry.idx)
  }
...
function main(): void {
  ...
  if (base === null) {
    warnings.push(
      `${BASE_REF} não encontrado — a checagem contra a base foi pulada (rode \`git fetch\`)`
    )
  } else {
  ...
  console.info(
    `[migrations] journal ok — ${entries.length} migrations em ordem`
  )
}
