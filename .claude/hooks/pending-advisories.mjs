...
import { parseInstalledVersion, readTemplateOrigin } from "../../scripts/platform/lib/template-version.mjs";
...
try {
  ...
  const { noLock, pending } = computePending(lock, advisories, ledger, { templateVersion });

  const formatAdvisory = (advisory) => `${advisory.id} ${advisory.kind} ${advisory.severity} ${advisory.module}`;
  const kernelLines = pending.filter((advisory) => advisory.module === "kernel").map(formatAdvisory);
  const entryLines = noLock
    ? ["no .platform-modules.lock — run platform module adopt"]
    : pending.filter((advisory) => advisory.module !== "kernel").map(formatAdvisory);
